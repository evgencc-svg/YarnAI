...
(function initializeProjectSystem(global) {
  ...
  function validateImportedPatternEvolutionInitiation(state, sourceProjectId) {
    const invalid = (code, message) => { throw new ProjectRepositoryError(code, message); };
    const api = global.YarnAIPatternEvolutionInitiation;
    if (
      !state || state.type !== "PATTERN_EVOLUTION_INITIATION" || state.kind !== "PATTERN_EVOLUTION_INITIATION" ||
      state.schemaVersion !== 1 || state.version !== 1 || state.projectId !== sourceProjectId ||
      !state.id || state.initiationId !== state.id || !state.patternId || !state.calculationId ||
      !state.sourceClosureId || !state.sourceRolloutEvaluationId || !state.sourceRolloutId || !state.sourcePromotionId ||
      !state.sourceValidationId || !state.sourceAdaptationId || !state.sourceLearningId || !state.sourceRetrospectiveId ||
      !state.sourceProof || !state.sourceSnapshots || !state.sourceDigest || !state.readiness || !state.identity ||
      !Array.isArray(state.successCriteria) || !Array.isArray(state.audit) ||
      !Number.isInteger(state.revision) || state.revision < 1 || !Number.isInteger(state.epoch) || state.epoch < 1
    ) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_INITIATION", "Imported pattern evolution initiation is corrupted.");
    if (!api?.validatePatternEvolutionInitiation) invalid("PATTERN_EVOLUTION_INITIATION_API_MISSING", "Pattern evolution initiation module is not loaded.");
    const report = api.validatePatternEvolutionInitiation(state);
    if (!report.valid) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_INITIATION", "Pattern evolution initiation failed domain validation.");
  }

  function validateImportedPatternEvolutionProposal(state, sourceProjectId) {
    const invalid = (code, message) => { throw new ProjectRepositoryError(code, message); };
    const api = global.YarnAIPatternEvolutionProposal;
    if (
      !state || state.type !== "PATTERN_EVOLUTION_PROPOSAL" || state.kind !== "PATTERN_EVOLUTION_PROPOSAL" ||
      state.schemaVersion !== 1 || state.version !== 1 || state.projectId !== sourceProjectId ||
      !state.id || state.proposalId !== state.id || !state.patternId || !state.calculationId ||
      !state.sourceInitiationId || !state.sourceClosureId || !state.sourceInitiationRevision || !state.sourceInitiationDigest ||
      !state.sourceProof || !state.sourceSnapshot || !state.traceability || !state.readiness || !state.identity ||
      !Array.isArray(state.proposedChanges) || !Array.isArray(state.audit) ||
      !Number.isInteger(state.revision) || state.revision < 1 || !Number.isInteger(state.epoch) || state.epoch < 1
    ) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL", "Imported pattern evolution proposal is corrupted.");
    if (!api?.validatePatternEvolutionProposal) invalid("PATTERN_EVOLUTION_PROPOSAL_API_MISSING", "Pattern evolution proposal module is not loaded.");
    const report = api.validatePatternEvolutionProposal(state);
    if (!report.valid) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL", "Pattern evolution proposal failed domain validation.");
  }

  function validateImportedPatternEvolutionProposalReview(state, sourceProjectId) {
    const invalid = (code, message) => { throw new ProjectRepositoryError(code, message); };
    const api = global.YarnAIPatternEvolutionProposalReview;
    if (
      !state || state.type !== "PATTERN_EVOLUTION_PROPOSAL_REVIEW" || state.kind !== "PATTERN_EVOLUTION_PROPOSAL_REVIEW" ||
      state.schemaVersion !== 1 || state.version !== 1 || state.projectId !== sourceProjectId ||
      !state.id || state.reviewId !== state.id || !state.patternId || !state.calculationId ||
      !state.sourceProposalId || !state.sourceInitiationId || !state.sourceProposalSnapshot ||
      !state.sourceProof || !state.sourceBinding || !state.dimensions || !state.readiness || !state.risk || !state.identity ||
      !Array.isArray(state.findings) || !Array.isArray(state.audit) ||
      !Number.isInteger(state.revision) || state.revision < 1 || !Number.isInteger(state.epoch) || state.epoch < 1
    ) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL_REVIEW", "Imported pattern evolution proposal review is corrupted.");
    if (!api?.validatePatternEvolutionProposalReview) invalid("PATTERN_EVOLUTION_PROPOSAL_REVIEW_API_MISSING", "Pattern evolution proposal review module is not loaded.");
    const report = api.validatePatternEvolutionProposalReview(state);
    if (!report.valid) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL_REVIEW", "Imported pattern evolution proposal review failed domain validation.");
  }

  function validateImportedPatternEvolutionDecision(state, sourceProjectId) {
    const invalid = (code, message) => { throw new ProjectRepositoryError(code, message); };
    const api = global.YarnAIPatternEvolutionDecision;
    if (
      !state || state.type !== "PATTERN_EVOLUTION_DECISION" || state.kind !== "PATTERN_EVOLUTION_DECISION" ||
      state.schemaVersion !== 1 || state.version !== 1 || state.projectId !== sourceProjectId ||
      !state.id || state.decisionId !== state.id || !state.patternId || !state.calculationId ||
      !state.sourceInitiationId || !state.sourceProposalId || !state.sourceReviewId ||
      !state.sourceInitiationRevision || !state.sourceProposalRevision || !state.sourceReviewRevision ||
      !state.sourceInitiationDigest || !state.sourceProposalDigest || !state.sourceReviewDigest ||
      !state.sourceSnapshots?.initiation || !state.sourceSnapshots?.proposal || !state.sourceSnapshots?.review ||
      !state.sourceProof || !state.sourceBinding || !state.identity || !state.digest ||
      !Array.isArray(state.reasons) || !Array.isArray(state.conditions) || !Array.isArray(state.audit) ||
      !Number.isInteger(state.revision) || state.revision < 1 || !Number.isInteger(state.epoch) || state.epoch < 1
    ) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_DECISION", "Imported pattern evolution decision is corrupted.");
    if (!api?.validatePatternEvolutionDecision) invalid("PATTERN_EVOLUTION_DECISION_API_MISSING", "Pattern evolution decision module is not loaded.");
    const report = api.validatePatternEvolutionDecision(state);
    if (!report.valid) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_DECISION", "Pattern evolution decision failed domain validation.");
  }

  function validateImportedPatternEvolutionExecution(state, sourceProjectId) {
    const invalid = (code, message) => { throw new ProjectRepositoryError(code, message); };
    const api = global.YarnAIPatternEvolutionExecution;
    if (
      !state || state.type !== "PATTERN_EVOLUTION_EXECUTION" || state.kind !== "PATTERN_EVOLUTION_EXECUTION" ||
      state.schemaVersion !== 1 || state.version !== 1 || state.projectId !== sourceProjectId ||
      !state.id || state.executionId !== state.id || !state.patternId || !state.calculationId ||
      !state.sourceInitiationId || !state.sourceProposalId || !state.sourceReviewId || !state.sourceDecisionId ||
      !state.sourceInitiationRevision || !state.sourceProposalRevision || !state.sourceReviewRevision || !state.sourceDecisionRevision ||
      !state.sourceInitiationDigest || !state.sourceProposalDigest || !state.sourceReviewDigest || !state.sourceDecisionDigest ||
      !state.sourceSnapshots?.initiation || !state.sourceSnapshots?.proposal || !state.sourceSnapshots?.review || !state.sourceSnapshots?.decision ||
      !state.sourceProof || !state.sourceEnvelope || !state.executionPlan || !state.identity || !state.semanticIdentity || !state.digest ||
      !Array.isArray(state.observations) || !Array.isArray(state.evidence) || !Array.isArray(state.reasons) || !Array.isArray(state.reasonCodes) || !Array.isArray(state.audit) ||
      !Number.isInteger(state.revision) || state.revision < 1 || !Number.isInteger(state.epoch) || state.epoch < 1
    ) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION", "Imported pattern evolution execution is corrupted.");
    if (!api?.validatePatternEvolutionExecution) invalid("PATTERN_EVOLUTION_EXECUTION_API_MISSING", "Pattern evolution execution module is not loaded.");
    const report = api.validatePatternEvolutionExecution(state);
    if (!report.valid) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION", "Pattern evolution execution failed domain validation.");
  }
  function validateImportedPatternEvolutionExecutionVerification(state, sourceProjectId) {
    const invalid = (code, message) => { throw new ProjectRepositoryError(code, message); };
    const api = global.YarnAIPatternEvolutionExecutionVerification;
    if (
      !state || state.type !== "PATTERN_EVOLUTION_EXECUTION_VERIFICATION" || state.kind !== "PATTERN_EVOLUTION_EXECUTION_VERIFICATION" ||
      state.schemaVersion !== 1 || state.version !== 1 || state.projectId !== sourceProjectId ||
      !state.id || state.verificationId !== state.id || !state.patternId || !state.calculationId ||
      !state.sourceInitiationId || !state.sourceProposalId || !state.sourceReviewId || !state.sourceDecisionId || !state.sourceExecutionId ||
      !state.sourceExecutionRevision || !state.sourceExecutionDigest || !state.sourceChainDigest ||
      !state.sourceSnapshots?.initiation || !state.sourceSnapshots?.proposal || !state.sourceSnapshots?.review || !state.sourceSnapshots?.decision || !state.sourceSnapshots?.execution ||
      !state.sourceProof || !state.sourceBinding || !state.verificationScope || !state.verificationContract || !state.identity || !state.semanticIdentity || !state.digest ||
      !Array.isArray(state.reasons) || !Array.isArray(state.reasonCodes) || !Array.isArray(state.audit) ||
      !Number.isInteger(state.revision) || state.revision < 1 || !Number.isInteger(state.epoch) || state.epoch < 1
    ) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION_VERIFICATION", "Imported execution verification is corrupted.");
    if (!api?.validatePatternEvolutionExecutionVerification) invalid("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_API_MISSING", "Pattern evolution execution verification module is not loaded.");
    const report = api.validatePatternEvolutionExecutionVerification(state);
    if (!report.valid) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION_VERIFICATION", "Pattern evolution execution verification failed domain validation.");
  }

  function validateImportedPatternEvolutionAcceptance(state, sourceProjectId) {
    const invalid = (code, message) => { throw new ProjectRepositoryError(code, message); };
    const api = global.YarnAIPatternEvolutionAcceptance;
    if (
      !state || state.type !== "PATTERN_EVOLUTION_ACCEPTANCE" || state.kind !== "PATTERN_EVOLUTION_ACCEPTANCE" ||
      state.schemaVersion !== 1 || state.version !== 1 || state.projectId !== sourceProjectId ||
      !state.id || state.acceptanceId !== state.id || !state.patternId || !state.calculationId ||
      !state.sourceInitiationId || !state.sourceProposalId || !state.sourceReviewId || !state.sourceDecisionId ||
      !state.sourceExecutionId || !state.sourceVerificationId || !state.sourceVerificationRevision ||
      !state.sourceVerificationDigest || !state.sourceChainDigest || !state.sourceSnapshots?.verification ||
      !state.sourceSnapshots?.execution || !state.sourceSnapshots?.decision || !state.sourceSnapshots?.review ||
      !state.sourceSnapshots?.proposal || !state.sourceSnapshots?.initiation || !state.sourceProof ||
      !state.sourceBinding || !state.assessmentInput || !state.acceptanceContract || !state.risk ||
      !state.identity || !state.semanticIdentity || !state.digest || !Array.isArray(state.reasons) ||
      !Array.isArray(state.reasonCodes) || !Array.isArray(state.audit) ||
      !Number.isInteger(state.revision) || state.revision < 1 || !Number.isInteger(state.epoch) || state.epoch < 1
    ) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_ACCEPTANCE", "Imported pattern evolution acceptance is corrupted.");
    if (!api?.validatePatternEvolutionAcceptance) invalid("PATTERN_EVOLUTION_ACCEPTANCE_API_MISSING", "Pattern evolution acceptance module is not loaded.");
    const report = api.validatePatternEvolutionAcceptance(state);
    if (!report.valid) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_ACCEPTANCE", "Pattern evolution acceptance failed domain validation.");
  }

  function validateImportedPatternEvolutionClosure(state, sourceProjectId) {
    const invalid = (code, message) => { throw new ProjectRepositoryError(code, message); };
    const api = global.YarnAIPatternEvolutionClosure;
    if (
      !state || state.type !== "PATTERN_EVOLUTION_CLOSURE" || state.kind !== "PATTERN_EVOLUTION_CLOSURE" ||
      state.persistenceKind !== "pattern_evolution_closure" || state.schemaVersion !== 1 || state.version !== 1 || state.projectId !== sourceProjectId ||
      !state.id || state.closureId !== state.id || !state.patternId || !state.calculationId || !state.evolutionId || !state.cycleId ||
      !state.sourceInitiationId || !state.sourceProposalId || !state.sourceReviewId || !state.sourceDecisionId ||
      !state.sourceExecutionId || !state.sourceVerificationId || !state.sourceAcceptanceId || !state.sourceAcceptanceRevision ||
      !state.sourceAcceptanceDigest || !state.sourceChainDigest || !state.sourceSnapshots?.acceptance ||
      !state.sourceSnapshots?.verification || !state.sourceSnapshots?.execution || !state.sourceSnapshots?.decision ||
      !state.sourceSnapshots?.review || !state.sourceSnapshots?.proposal || !state.sourceSnapshots?.initiation ||
      !state.sourceProof || !state.sourceBinding || !state.reconciliationInput || !state.closureContract || !state.risk ||
      !state.identity || !state.semanticIdentity || !state.digest || !state.closureDigest || !Array.isArray(state.reasons) ||
      !Array.isArray(state.reasonCodes) || !Array.isArray(state.audit) ||
      !Number.isInteger(state.revision) || state.revision < 1 || !Number.isInteger(state.epoch) || state.epoch < 1
    ) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_CLOSURE", "Imported pattern evolution closure is corrupted.");
    if (!api?.validatePatternEvolutionClosure) invalid("PATTERN_EVOLUTION_CLOSURE_API_MISSING", "Pattern evolution closure module is not loaded.");
    const report = api.validatePatternEvolutionClosure(state);
    if (!report.valid) invalid("INVALID_IMPORT_PATTERN_EVOLUTION_CLOSURE", "Imported pattern evolution closure failed domain validation.");
  }
  ...
  class ProjectRepository {
    ...
    async _quarantinePatternEvolutionInitiation(record, reasonCode) {
      const api = global.YarnAIPatternEvolutionInitiation; const identity = api?.fingerprint ? api.fingerprint({ progressId: record?.progress_id || null, state: record?.state || null, reasonCode }) : canonicalize({ progressId: record?.progress_id || null, reasonCode }); const timestamp = record?.state?.updatedAt || record?.updated_at || "1970-01-01T00:00:00.000Z";
      const quarantine = { quarantine_id: `pattern-evolution-initiation-quarantine:${identity}`, source_store: "progress", source_key: record?.progress_id || null, reason_code: reasonCode, record: clone(record), created_at: timestamp, expires_at: timestamp }; const database = await this._database(); const transaction = database.transaction("quarantine", "readwrite"); transaction.objectStore("quarantine").put(quarantine); await transactionComplete(transaction); return clone(quarantine);
    }

    async listPatternEvolutionInitiations(projectId, calculationId = null, closureId = null, filters = {}) {
      if (calculationId && typeof calculationId === "object") { filters = calculationId; calculationId = filters.calculationId || null; closureId = filters.closureId || filters.sourceClosureId || null; }
      const project = await this._validatedCurrentProject(projectId); const effectiveCalculationId = calculationId || project.active_calculation_id; if (!effectiveCalculationId) return []; const database = await this._database(); const transaction = database.transaction("progress", "readonly"); const records = await requestResult(transaction.objectStore("progress").index("by_calculation_kind").getAll(global.IDBKeyRange.only([effectiveCalculationId, "PATTERN_EVOLUTION_INITIATION"]))); await transactionComplete(transaction);
      const filtered = records.filter((entry) => entry.project_id === projectId && (!closureId || entry.state?.sourceClosureId === closureId) && (!filters.patternId || entry.state?.patternId === filters.patternId) && (!filters.status || entry.state?.status === filters.status) && (!filters.verdict || entry.state?.verdict === filters.verdict) && (filters.stale === undefined || entry.state?.stale === filters.stale) && (!filters.proofStatus || entry.state?.proofStatus === filters.proofStatus) && (filters.importedUnproven === undefined || entry.state?.importedUnproven === filters.importedUnproven) && (filters.collision === undefined || entry.state?.collision === filters.collision)); filtered.sort((a, b) => a.epoch - b.epoch || a.state.revision - b.state.revision || (String(a.state.id) < String(b.state.id) ? -1 : String(a.state.id) > String(b.state.id) ? 1 : 0)); const valid = [];
      for (const entry of filtered) { try { validateProgressRecord(entry, projectId, effectiveCalculationId, "PATTERN_EVOLUTION_INITIATION"); validateImportedPatternEvolutionInitiation(entry.state, projectId); valid.push(clone(entry)); } catch (error) { await this._quarantinePatternEvolutionInitiation(entry, error?.code || "INVALID_PATTERN_EVOLUTION_INITIATION"); } } return valid;
    }
    async getPatternEvolutionInitiation(projectId, initiationId = null, calculationId = null, closureId = null) { const records = await this.listPatternEvolutionInitiations(projectId, calculationId, closureId); if (initiationId) return records.find((entry) => entry.progress_id === initiationId || entry.state?.id === initiationId || entry.state?.initiationId === initiationId) || null; return records.at(-1) || null; }
    async getLatestPatternEvolutionInitiation(projectId, calculationId = null, closureId = null, filters = {}) { const records = calculationId && typeof calculationId === "object" ? await this.listPatternEvolutionInitiations(projectId, calculationId) : await this.listPatternEvolutionInitiations(projectId, calculationId, closureId, filters); return records.at(-1) || null; }

    async _assertPatternEvolutionInitiationReferences(projectId, calculationId, state, initiationStorageRevision = 0) {
      const api = global.YarnAIPatternEvolutionInitiation; if (!api?.loadSource || !api?.calculateEvidenceProof) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_API_MISSING", "Pattern evolution initiation module is not loaded."); const source = await api.loadSource(this, projectId, state.sourceClosureId); if (source.calculationId !== calculationId) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_SCOPE_INVALID", "Evolution initiation does not belong to the active calculation."); const proof = api.calculateEvidenceProof({ ...source, initiationStorageRevision }, state); const staleRecord = state.status === "stale" || state.proofStatus === "imported-unproven"; if (!proof.recordsResolved || !proof.sameProject || !proof.samePattern || !proof.linksValid || !proof.identitiesMatch || !proof.closureTerminal || (!staleRecord && (!proof.snapshotsMatch || !proof.sourceDigestValid)) || !proof.collisionFree) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_REFERENCE_INVALID", "Evolution initiation references do not resolve to one local evidence chain.", { details: { issues: proof.issues } }); return { source, proof };
    }

    async savePatternEvolutionInitiation(projectId, calculationOrState, stateOrOptions = {}, maybeOptions = {}) {
      const directState = calculationOrState && typeof calculationOrState === "object"; const state = directState ? calculationOrState : stateOrOptions; const calculationId = directState ? state.calculationId : calculationOrState; const options = directState ? stateOrOptions : maybeOptions; const api = global.YarnAIPatternEvolutionInitiation; const report = api?.validatePatternEvolutionInitiation?.(state); if (!api || state?.kind !== "PATTERN_EVOLUTION_INITIATION" || state?.type !== "PATTERN_EVOLUTION_INITIATION" || state?.projectId !== projectId || state?.calculationId !== calculationId || !report?.valid) throw new ProjectRepositoryError("INVALID_PATTERN_EVOLUTION_INITIATION_STATE", "Pattern evolution initiation snapshot is corrupted."); validateImportedPatternEvolutionInitiation(state, projectId);
      return this._serialize(projectId, async () => {
        const before = await this._validatedCurrentProject(projectId); if (before.active_calculation_id !== calculationId) throw new ProjectRepositoryError("CALCULATION_MISMATCH", "Evolution initiation does not belong to the active calculation."); const existing = await this.listPatternEvolutionInitiations(projectId, calculationId); if (existing.some((entry) => entry.state?.identity === state.identity && entry.state?.id !== state.id)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_IDENTITY_COLLISION", "Evolution initiation identity collides with another record."); const current = options.recordId ? existing.find((entry) => entry.progress_id === options.recordId || entry.state?.id === options.recordId) : existing.find((entry) => entry.state?.id === state.id); await this._assertPatternEvolutionInitiationReferences(projectId, calculationId, state, current?.revision || 0); const timestamp = options.timestamp || state.updatedAt;
        if (state.status === "approved" && existing.some((entry) => entry.state?.id !== state.id && entry.state?.status === "approved" && entry.state?.sourceClosureId === state.sourceClosureId && canonicalize(entry.state?.evolutionScope) === canonicalize(state.evolutionScope))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_DUPLICATE_APPROVED", "An approved initiative already exists for this closure and scope.");
        if (current) {
          if ((options.expectedRevision !== undefined && options.expectedRevision !== current.state.revision) || (options.expectedIdentity !== undefined && options.expectedIdentity !== current.state.identity)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_REVISION_CONFLICT", "Evolution initiation was changed by another operation."); if (canonicalize(state) === canonicalize(current.state)) return clone(current); const revalidation = options.allowRevalidation === true && options.operationKind === "PATTERN_EVOLUTION_INITIATION_REVALIDATED"; if (["approved", "rejected", "cancelled"].includes(current.state.status) && !revalidation) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_TERMINAL", "Terminal evolution initiation is immutable."); if (state.id !== current.state.id || state.initiationId !== current.state.initiationId || state.epoch !== current.state.epoch || state.revision <= current.state.revision) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_REVISION_CONFLICT", "Evolution initiation revision or identity is invalid."); const allowed = { draft: ["draft", "assessing", "cancelled", "stale"], assessing: ["assessing", "ready", "rejected", "cancelled", "stale"], ready: ["ready", "approved", "rejected", "cancelled", "stale"], stale: ["stale", "draft"] }; if (!revalidation && !allowed[current.state.status]?.includes(state.status)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_LIFECYCLE_CONFLICT", "Evolution initiation lifecycle transition is invalid."); const immutableFields = ["projectId", "patternId", "calculationId", "sourceClosureId", "sourceRolloutEvaluationId", "sourceRolloutId", "sourcePromotionId", "sourceValidationId", "sourceAdaptationId", "sourceLearningId", "sourceRetrospectiveId", "sourceSnapshots", "sourceDigest", "epoch", "createdAt"]; if (!revalidation && immutableFields.some((field) => canonicalize(state[field]) !== canonicalize(current.state[field]))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_SOURCE_IMMUTABLE", "Evolution initiation source chain is immutable.");
          const nextProgress = clone(current); nextProgress.state = clone(state); nextProgress.revision += 1; nextProgress.updated_at = timestamp; const nextProject = clone(before); nextProject.current_stage = `pattern_evolution_initiation_${state.status}`; nextProject.updated_at = timestamp; nextProject.revision += 1; nextProject.materialized_checksum = await checksumPayload(projectChecksumPayload(nextProject)); const operation = createOperation(nextProject, options.operationKind || `PATTERN_EVOLUTION_INITIATION_${state.status.toUpperCase()}`, { calculation_id: calculationId, progress_id: current.progress_id, progress_epoch: current.epoch, progress_revision: nextProgress.revision, progress_state: clone(state) }, timestamp, before.revision, nextProject.revision); const checkpoint = createCheckpoint(nextProject, nextProject.materialized_checksum, nextProject.revision, timestamp); const database = await this._database(); const transaction = database.transaction(["projects", "progress", "operations", "checkpoints", "meta"], "readwrite"); try { const storedProject = await requestResult(transaction.objectStore("projects").get(projectId)); const storedProgress = await requestResult(transaction.objectStore("progress").get(current.progress_id)); if (!storedProject || storedProject.revision !== before.revision || !storedProgress || storedProgress.revision !== current.revision) { transaction.abort(); throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_REVISION_CONFLICT", "Evolution initiation changed concurrently."); } await allocateOperationMetadata(transaction, operation); transaction.objectStore("progress").put(nextProgress); transaction.objectStore("projects").put(nextProject); transaction.objectStore("operations").add(operation); transaction.objectStore("checkpoints").add(checkpoint); await transactionComplete(transaction); } catch (error) { throw mapStorageError(error); } this._notify(projectId, nextProject.revision, operation.kind); return clone(nextProgress);
        }
        const progress = this._initialProgress(projectId, calculationId, "PATTERN_EVOLUTION_INITIATION", timestamp); progress.epoch = existing.reduce((maximum, entry) => Math.max(maximum, entry.epoch), 0) + 1; progress.state = clone(state); if (progress.state.epoch !== progress.epoch) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_EPOCH_CONFLICT", "Evolution initiation epoch differs from progress epoch."); const nextProject = clone(before); nextProject.current_stage = `pattern_evolution_initiation_${state.status}`; nextProject.updated_at = timestamp; nextProject.revision += 1; nextProject.materialized_checksum = await checksumPayload(projectChecksumPayload(nextProject)); const operation = createOperation(nextProject, options.operationKind || "PATTERN_EVOLUTION_INITIATION_CREATED", { calculation_id: calculationId, progress_id: progress.progress_id, progress_epoch: progress.epoch, progress_revision: progress.revision, progress_state: clone(state) }, timestamp, before.revision, nextProject.revision); const checkpoint = createCheckpoint(nextProject, nextProject.materialized_checksum, nextProject.revision, timestamp); const database = await this._database(); const transaction = database.transaction(["projects", "progress", "operations", "checkpoints", "meta"], "readwrite"); try { const storedProject = await requestResult(transaction.objectStore("projects").get(projectId)); if (!storedProject || storedProject.revision !== before.revision) { transaction.abort(); throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_REVISION_CONFLICT", "Evolution initiation changed concurrently."); } await allocateOperationMetadata(transaction, operation); transaction.objectStore("progress").add(progress); transaction.objectStore("projects").put(nextProject); transaction.objectStore("operations").add(operation); transaction.objectStore("checkpoints").add(checkpoint); await transactionComplete(transaction); } catch (error) { throw mapStorageError(error); } this._notify(projectId, nextProject.revision, operation.kind); return clone(progress);
      });
    }
    async createPatternEvolutionInitiation(projectId, input = {}) { const api = global.YarnAIPatternEvolutionInitiation; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_API_MISSING", "Pattern evolution initiation module is not loaded."); return api.createForProject(this, projectId, input); }
    async readPatternEvolutionInitiation(projectId, initiationId = null, closureId = null) { const api = global.YarnAIPatternEvolutionInitiation; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_API_MISSING", "Pattern evolution initiation module is not loaded."); return api.readForProject(this, projectId, initiationId, closureId); }
    async revalidatePatternEvolutionInitiation(projectId, initiationId, options = {}) { const current = await this.getPatternEvolutionInitiation(projectId, initiationId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_NOT_FOUND", "Pattern evolution initiation was not found."); const api = global.YarnAIPatternEvolutionInitiation; const source = await api.loadSource(this, projectId, current.state.sourceClosureId); const next = api.revalidatePatternEvolutionInitiation(current.state, source, { ...options, now: options.timestamp || current.state.updatedAt }); return this.savePatternEvolutionInitiation(projectId, next, { ...options, recordId: current.progress_id, expectedRevision: current.state.revision, expectedIdentity: current.state.identity, operationKind: "PATTERN_EVOLUTION_INITIATION_REVALIDATED", allowRevalidation: true }); }
    async _quarantinePatternEvolutionProposal(record, reasonCode) {
      const api = global.YarnAIPatternEvolutionProposal;
      const identity = api?.fingerprint ? api.fingerprint({ progressId: record?.progress_id || null, state: record?.state || null, reasonCode }) : canonicalize({ progressId: record?.progress_id || null, reasonCode });
      const timestamp = record?.state?.updatedAt || record?.updated_at || "1970-01-01T00:00:00.000Z";
      const quarantine = { quarantine_id: `pattern-evolution-proposal-quarantine:${identity}`, source_store: "progress", source_key: record?.progress_id || null, reason_code: reasonCode, record: clone(record), created_at: timestamp, expires_at: timestamp };
      const database = await this._database(); const transaction = database.transaction("quarantine", "readwrite"); transaction.objectStore("quarantine").put(quarantine); await transactionComplete(transaction); return clone(quarantine);
    }

    async listPatternEvolutionProposals(projectId, calculationId = null, initiationId = null, filters = {}) {
      if (calculationId && typeof calculationId === "object") { filters = calculationId; calculationId = filters.calculationId || null; initiationId = filters.initiationId || filters.sourceInitiationId || null; }
      const project = await this._validatedCurrentProject(projectId); const effectiveCalculationId = calculationId || project.active_calculation_id; if (!effectiveCalculationId) return [];
      const database = await this._database(); const transaction = database.transaction("progress", "readonly"); const records = await requestResult(transaction.objectStore("progress").index("by_calculation_kind").getAll(global.IDBKeyRange.only([effectiveCalculationId, "PATTERN_EVOLUTION_PROPOSAL"]))); await transactionComplete(transaction);
      const filtered = records.filter((entry) => entry.project_id === projectId && (!initiationId || entry.state?.sourceInitiationId === initiationId) && (!filters.patternId || entry.state?.patternId === filters.patternId) && (!filters.status || entry.state?.status === filters.status) && (!filters.verdict || entry.state?.verdict === filters.verdict) && (filters.stale === undefined || entry.state?.stale === filters.stale) && (!filters.proofStatus || entry.state?.proofStatus === filters.proofStatus));
      filtered.sort((a, b) => a.epoch - b.epoch || a.state.revision - b.state.revision || (String(a.state.id) < String(b.state.id) ? -1 : String(a.state.id) > String(b.state.id) ? 1 : 0)); const valid = [];
      for (const entry of filtered) { try { validateProgressRecord(entry, projectId, effectiveCalculationId, "PATTERN_EVOLUTION_PROPOSAL"); validateImportedPatternEvolutionProposal(entry.state, projectId); valid.push(clone(entry)); } catch (error) { await this._quarantinePatternEvolutionProposal(entry, error?.code || "INVALID_PATTERN_EVOLUTION_PROPOSAL"); } } return valid;
    }

    async getPatternEvolutionProposal(projectId, proposalId = null, calculationId = null, initiationId = null) { const records = await this.listPatternEvolutionProposals(projectId, calculationId, initiationId); if (proposalId) return records.find((entry) => entry.progress_id === proposalId || entry.state?.id === proposalId || entry.state?.proposalId === proposalId) || null; return records.at(-1) || null; }
    async getLatestPatternEvolutionProposal(projectId, calculationId = null, initiationId = null, filters = {}) { const records = calculationId && typeof calculationId === "object" ? await this.listPatternEvolutionProposals(projectId, calculationId) : await this.listPatternEvolutionProposals(projectId, calculationId, initiationId, filters); return records.at(-1) || null; }

    async _assertPatternEvolutionProposalReferences(projectId, calculationId, state) {
      const api = global.YarnAIPatternEvolutionProposal; if (!api?.loadSource || !api?.calculateSourceProof) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_API_MISSING", "Pattern evolution proposal module is not loaded.");
      const source = await api.loadSource(this, projectId, state.sourceInitiationId); if (source.calculationId !== calculationId) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_SCOPE_INVALID", "Evolution proposal does not belong to the active calculation.");
      const proof = api.calculateSourceProof(source, state); const staleRecord = state.status === "stale" || state.proofStatus === "imported-unproven"; if (!proof.sourceResolved || !proof.sourceApproved || !proof.sameProject || !proof.samePattern || !proof.sourceIdMatches || (!staleRecord && (!proof.revisionCurrent || !proof.digestCurrent || !proof.snapshotMatches || !proof.provenanceVerified))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REFERENCE_INVALID", "Evolution proposal does not resolve to one approved local initiation.", { details: { issues: proof.issues } }); return { source, proof };
    }

    async savePatternEvolutionProposal(projectId, calculationOrState, stateOrOptions = {}, maybeOptions = {}) {
      const directState = calculationOrState && typeof calculationOrState === "object"; const state = directState ? calculationOrState : stateOrOptions; const calculationId = directState ? state.calculationId : calculationOrState; const options = directState ? stateOrOptions : maybeOptions;
      const api = global.YarnAIPatternEvolutionProposal; const report = api?.validatePatternEvolutionProposal?.(state); if (!api || state?.kind !== "PATTERN_EVOLUTION_PROPOSAL" || state?.type !== "PATTERN_EVOLUTION_PROPOSAL" || state?.projectId !== projectId || state?.calculationId !== calculationId || !report?.valid) throw new ProjectRepositoryError("INVALID_PATTERN_EVOLUTION_PROPOSAL_STATE", "Pattern evolution proposal snapshot is corrupted."); validateImportedPatternEvolutionProposal(state, projectId);
      return this._serialize(projectId, async () => {
        const before = await this._validatedCurrentProject(projectId); if (before.active_calculation_id !== calculationId) throw new ProjectRepositoryError("CALCULATION_MISMATCH", "Evolution proposal does not belong to the active calculation.");
        const existing = await this.listPatternEvolutionProposals(projectId, calculationId); if (existing.some((entry) => entry.state?.identity === state.identity && entry.state?.id !== state.id)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_IDENTITY_COLLISION", "Evolution proposal identity collides with another record.");
        if (state.status === "submitted" && api.duplicateSubmitted(state, existing.map((entry) => entry.state))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_DUPLICATE_SUBMITTED", "A submitted proposal already exists for this initiation, target and change set.");
        const current = options.recordId ? existing.find((entry) => entry.progress_id === options.recordId || entry.state?.id === options.recordId) : existing.find((entry) => entry.state?.id === state.id); await this._assertPatternEvolutionProposalReferences(projectId, calculationId, state); const timestamp = options.timestamp || state.updatedAt;
        let progress; let operationKind;
        if (current) {
          if ((options.expectedRevision !== undefined && options.expectedRevision !== current.state.revision) || (options.expectedIdentity !== undefined && options.expectedIdentity !== current.state.identity)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVISION_CONFLICT", "Evolution proposal was changed by another operation."); if (canonicalize(state) === canonicalize(current.state)) return clone(current);
          const revalidation = options.allowRevalidation === true && options.operationKind === "PATTERN_EVOLUTION_PROPOSAL_REVALIDATED"; if (["submitted", "rejected", "cancelled"].includes(current.state.status) && !revalidation) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_TERMINAL", "Terminal evolution proposal is immutable.");
          if (state.id !== current.state.id || state.proposalId !== current.state.proposalId || state.epoch !== current.state.epoch || state.revision <= current.state.revision) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVISION_CONFLICT", "Evolution proposal revision or identity is invalid.");
          const allowed = { draft: ["draft", "composing", "cancelled", "stale"], composing: ["composing", "ready", "rejected", "cancelled", "stale"], ready: ["ready", "submitted", "rejected", "cancelled", "stale"], stale: ["stale", "draft"] }; if (!revalidation && !allowed[current.state.status]?.includes(state.status)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_LIFECYCLE_CONFLICT", "Evolution proposal lifecycle transition is invalid.");
          const immutableFields = ["projectId", "patternId", "calculationId", "sourceInitiationId", "sourceClosureId", "sourceInitiationRevision", "sourceInitiationDigest", "sourceSnapshot", "sourceBinding", "epoch", "createdAt"]; if (!revalidation && immutableFields.some((field) => canonicalize(state[field]) !== canonicalize(current.state[field]))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_SOURCE_IMMUTABLE", "Evolution proposal source initiation is immutable.");
          progress = clone(current); progress.state = clone(state); progress.revision += 1; progress.updated_at = timestamp; operationKind = options.operationKind || `PATTERN_EVOLUTION_PROPOSAL_${state.status.toUpperCase()}`;
        } else {
          progress = this._initialProgress(projectId, calculationId, "PATTERN_EVOLUTION_PROPOSAL", timestamp); progress.epoch = existing.reduce((maximum, entry) => Math.max(maximum, entry.epoch), 0) + 1; progress.state = clone(state); if (progress.state.epoch !== progress.epoch) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_EPOCH_CONFLICT", "Evolution proposal epoch differs from progress epoch."); operationKind = options.operationKind || "PATTERN_EVOLUTION_PROPOSAL_CREATED";
        }
        const nextProject = clone(before); nextProject.current_stage = `pattern_evolution_proposal_${state.status}`; nextProject.updated_at = timestamp; nextProject.revision += 1; nextProject.materialized_checksum = await checksumPayload(projectChecksumPayload(nextProject));
        const operation = createOperation(nextProject, operationKind, { calculation_id: calculationId, progress_id: progress.progress_id, progress_epoch: progress.epoch, progress_revision: progress.revision, progress_state: clone(state) }, timestamp, before.revision, nextProject.revision); const checkpoint = createCheckpoint(nextProject, nextProject.materialized_checksum, nextProject.revision, timestamp);
        const database = await this._database(); const transaction = database.transaction(["projects", "progress", "operations", "checkpoints", "meta"], "readwrite"); try { const storedProject = await requestResult(transaction.objectStore("projects").get(projectId)); const storedProgress = current ? await requestResult(transaction.objectStore("progress").get(current.progress_id)) : null; if (!storedProject || storedProject.revision !== before.revision || (current && (!storedProgress || storedProgress.revision !== current.revision))) { transaction.abort(); throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVISION_CONFLICT", "Evolution proposal changed concurrently."); } await allocateOperationMetadata(transaction, operation); current ? transaction.objectStore("progress").put(progress) : transaction.objectStore("progress").add(progress); transaction.objectStore("projects").put(nextProject); transaction.objectStore("operations").add(operation); transaction.objectStore("checkpoints").add(checkpoint); await transactionComplete(transaction); } catch (error) { throw mapStorageError(error); }
        this._notify(projectId, nextProject.revision, operation.kind); return clone(progress);
      });
    }

    async createPatternEvolutionProposal(projectId, input = {}) { const api = global.YarnAIPatternEvolutionProposal; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_API_MISSING", "Pattern evolution proposal module is not loaded."); return api.createForProject(this, projectId, input); }
    async readPatternEvolutionProposal(projectId, proposalId = null, initiationId = null) { const api = global.YarnAIPatternEvolutionProposal; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_API_MISSING", "Pattern evolution proposal module is not loaded."); return api.readForProject(this, projectId, proposalId, initiationId); }
    async revalidatePatternEvolutionProposal(projectId, proposalId, options = {}) { const current = await this.getPatternEvolutionProposal(projectId, proposalId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_NOT_FOUND", "Pattern evolution proposal was not found."); const api = global.YarnAIPatternEvolutionProposal; const source = await api.loadSource(this, projectId, current.state.sourceInitiationId); const next = api.revalidatePatternEvolutionProposal(current.state, source, { ...options, now: options.timestamp || current.state.updatedAt }); return this.savePatternEvolutionProposal(projectId, next, { ...options, recordId: current.progress_id, expectedRevision: current.state.revision, expectedIdentity: current.state.identity, operationKind: "PATTERN_EVOLUTION_PROPOSAL_REVALIDATED", allowRevalidation: true }); }

    async _quarantinePatternEvolutionProposalReview(record, reasonCode) {
      const api = global.YarnAIPatternEvolutionProposalReview;
      const identity = api?.fingerprint ? api.fingerprint({ progressId: record?.progress_id || null, state: record?.state || null, reasonCode }) : canonicalize({ progressId: record?.progress_id || null, reasonCode });
      const timestamp = record?.state?.updatedAt || record?.updated_at || "1970-01-01T00:00:00.000Z";
      const quarantine = { quarantine_id: `pattern-evolution-proposal-review-quarantine:${identity}`, source_store: "progress", source_key: record?.progress_id || null, reason_code: reasonCode, record: clone(record), created_at: timestamp, expires_at: timestamp };
      const database = await this._database(); const transaction = database.transaction("quarantine", "readwrite"); transaction.objectStore("quarantine").put(quarantine); await transactionComplete(transaction); return clone(quarantine);
    }

    async listPatternEvolutionProposalReviews(projectId, calculationId = null, proposalId = null, filters = {}) {
      if (calculationId && typeof calculationId === "object") { filters = calculationId; calculationId = filters.calculationId || null; proposalId = filters.proposalId || filters.sourceProposalId || null; }
      const project = await this._validatedCurrentProject(projectId); const effectiveCalculationId = calculationId || project.active_calculation_id; if (!effectiveCalculationId) return [];
      const database = await this._database(); const transaction = database.transaction("progress", "readonly"); const records = await requestResult(transaction.objectStore("progress").index("by_calculation_kind").getAll(global.IDBKeyRange.only([effectiveCalculationId, "PATTERN_EVOLUTION_PROPOSAL_REVIEW"]))); await transactionComplete(transaction);
      const filtered = records.filter((entry) => entry.project_id === projectId && (!proposalId || entry.state?.sourceProposalId === proposalId) && (!filters.patternId || entry.state?.patternId === filters.patternId) && (!filters.status || entry.state?.status === filters.status) && (!filters.verdict || entry.state?.verdict === filters.verdict) && (!filters.proofStatus || entry.state?.proofStatus === filters.proofStatus) && (filters.stale === undefined || (entry.state?.status === "stale") === filters.stale));
      filtered.sort((a, b) => a.epoch - b.epoch || a.state.revision - b.state.revision || (String(a.state.id) < String(b.state.id) ? -1 : String(a.state.id) > String(b.state.id) ? 1 : 0)); const valid = [];
      for (const entry of filtered) { try { validateProgressRecord(entry, projectId, effectiveCalculationId, "PATTERN_EVOLUTION_PROPOSAL_REVIEW"); validateImportedPatternEvolutionProposalReview(entry.state, projectId); valid.push(clone(entry)); } catch (error) { await this._quarantinePatternEvolutionProposalReview(entry, error?.code || "INVALID_PATTERN_EVOLUTION_PROPOSAL_REVIEW"); } } return valid;
    }

    async getPatternEvolutionProposalReview(projectId, reviewId = null, calculationId = null, proposalId = null) { const records = await this.listPatternEvolutionProposalReviews(projectId, calculationId, proposalId); if (reviewId) return records.find((entry) => entry.progress_id === reviewId || entry.state?.id === reviewId || entry.state?.reviewId === reviewId) || null; return records.at(-1) || null; }
    async getLatestPatternEvolutionProposalReview(projectId, calculationId = null, proposalId = null, filters = {}) { const records = calculationId && typeof calculationId === "object" ? await this.listPatternEvolutionProposalReviews(projectId, calculationId) : await this.listPatternEvolutionProposalReviews(projectId, calculationId, proposalId, filters); return records.at(-1) || null; }

    async _assertPatternEvolutionProposalReviewReferences(projectId, calculationId, state) {
      const api = global.YarnAIPatternEvolutionProposalReview; if (!api?.loadSource || !api?.calculateSourceProof) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_API_MISSING", "Pattern evolution proposal review module is not loaded.");
      const source = await api.loadSource(this, projectId, state.sourceProposalId); if (source.calculationId !== calculationId) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_SCOPE_INVALID", "Proposal review does not belong to the active calculation."); const proof = api.calculateSourceProof(source, state); if (!proof.proposalResolved || !proof.sameProject || !proof.samePattern || !proof.proposalIdMatches || !proof.semanticEquivalent) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_REFERENCE_INVALID", "Proposal review does not resolve to one immutable local proposal.", { details: { issues: proof.issues } }); return { source, proof };
    }

    async savePatternEvolutionProposalReview(projectId, calculationOrState, stateOrOptions = {}, maybeOptions = {}) {
      const directState = calculationOrState && typeof calculationOrState === "object"; const state = directState ? calculationOrState : stateOrOptions; const calculationId = directState ? state.calculationId : calculationOrState; const options = directState ? stateOrOptions : maybeOptions; const api = global.YarnAIPatternEvolutionProposalReview; const report = api?.validatePatternEvolutionProposalReview?.(state);
      if (!api || state?.kind !== "PATTERN_EVOLUTION_PROPOSAL_REVIEW" || state?.type !== "PATTERN_EVOLUTION_PROPOSAL_REVIEW" || state?.projectId !== projectId || state?.calculationId !== calculationId || !report?.valid) throw new ProjectRepositoryError("INVALID_PATTERN_EVOLUTION_PROPOSAL_REVIEW_STATE", "Pattern evolution proposal review snapshot is corrupted."); validateImportedPatternEvolutionProposalReview(state, projectId);
      return this._serialize(projectId, async () => {
        const before = await this._validatedCurrentProject(projectId); if (before.active_calculation_id !== calculationId) throw new ProjectRepositoryError("CALCULATION_MISMATCH", "Proposal review does not belong to the active calculation."); const existing = await this.listPatternEvolutionProposalReviews(projectId, calculationId); const current = options.recordId ? existing.find((entry) => entry.progress_id === options.recordId || entry.state?.id === options.recordId) : existing.find((entry) => entry.state?.id === state.id); const revalidation = options.allowRevalidation === true && state.predecessorReviewId && existing.some((entry) => entry.state?.id === state.predecessorReviewId);
        if (!current && existing.some((entry) => entry.state?.identity === state.identity) && !revalidation) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_DUPLICATE_IDENTITY", "A review already exists for this immutable proposal revision.");
        if (!current && state.status === "approved" && existing.some((entry) => entry.state?.status === "approved" && entry.state?.identity === state.identity)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_DUPLICATE_APPROVED", "An approved review already exists for this proposal revision.");
        await this._assertPatternEvolutionProposalReviewReferences(projectId, calculationId, state); const timestamp = options.timestamp || state.updatedAt; let progress; let operationKind;
        if (current) {
          if ((options.expectedRevision !== undefined && options.expectedRevision !== current.state.revision) || (options.expectedIdentity !== undefined && options.expectedIdentity !== current.state.identity)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_REVISION_CONFLICT", "Proposal review was changed by another operation."); if (canonicalize(state) === canonicalize(current.state)) return clone(current); if (["approved", "changes_requested", "rejected", "cancelled"].includes(current.state.status)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_TERMINAL", "Terminal proposal reviews are immutable."); if (state.id !== current.state.id || state.reviewId !== current.state.reviewId || state.epoch !== current.state.epoch || state.revision <= current.state.revision) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_REVISION_CONFLICT", "Proposal review revision or identity is invalid."); const allowed = { draft: ["reviewing", "cancelled", "stale"], reviewing: ["ready", "changes_requested", "rejected", "cancelled", "stale"], ready: ["approved", "changes_requested", "rejected", "cancelled", "stale"], stale: [] }; if (!allowed[current.state.status]?.includes(state.status)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_LIFECYCLE_CONFLICT", "Proposal review lifecycle transition is invalid."); const immutableFields = ["projectId", "patternId", "calculationId", "sourceProposalId", "sourceProposalRevision", "sourceProposalDigest", "sourceInitiationId", "sourceProposalSnapshot", "policyVersion", "epoch", "createdAt"]; if (immutableFields.some((field) => canonicalize(state[field]) !== canonicalize(current.state[field]))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_SOURCE_IMMUTABLE", "Proposal review source snapshot is immutable."); progress = clone(current); progress.state = clone(state); progress.revision += 1; progress.updated_at = timestamp; operationKind = options.operationKind || `PATTERN_EVOLUTION_PROPOSAL_REVIEW_${state.status.toUpperCase()}`;
        } else { progress = this._initialProgress(projectId, calculationId, "PATTERN_EVOLUTION_PROPOSAL_REVIEW", timestamp); progress.epoch = existing.reduce((maximum, entry) => Math.max(maximum, entry.epoch), 0) + 1; progress.state = clone(state); if (progress.state.epoch !== progress.epoch) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_EPOCH_CONFLICT", "Proposal review epoch differs from progress epoch."); operationKind = options.operationKind || (revalidation ? "PATTERN_EVOLUTION_PROPOSAL_REVIEW_REVALIDATED" : "PATTERN_EVOLUTION_PROPOSAL_REVIEW_CREATED"); }
        const nextProject = clone(before); nextProject.current_stage = `pattern_evolution_proposal_review_${state.status}`; nextProject.updated_at = timestamp; nextProject.revision += 1; nextProject.materialized_checksum = await checksumPayload(projectChecksumPayload(nextProject)); const operation = createOperation(nextProject, operationKind, { calculation_id: calculationId, progress_id: progress.progress_id, progress_epoch: progress.epoch, progress_revision: progress.revision, progress_state: clone(state) }, timestamp, before.revision, nextProject.revision); const checkpoint = createCheckpoint(nextProject, nextProject.materialized_checksum, nextProject.revision, timestamp);
        const database = await this._database(); const transaction = database.transaction(["projects", "progress", "operations", "checkpoints", "meta"], "readwrite"); try { const storedProject = await requestResult(transaction.objectStore("projects").get(projectId)); const storedProgress = current ? await requestResult(transaction.objectStore("progress").get(current.progress_id)) : null; if (!storedProject || storedProject.revision !== before.revision || (current && (!storedProgress || storedProgress.revision !== current.revision))) { transaction.abort(); throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_REVISION_CONFLICT", "Proposal review changed concurrently."); } await allocateOperationMetadata(transaction, operation); current ? transaction.objectStore("progress").put(progress) : transaction.objectStore("progress").add(progress); transaction.objectStore("projects").put(nextProject); transaction.objectStore("operations").add(operation); transaction.objectStore("checkpoints").add(checkpoint); await transactionComplete(transaction); } catch (error) { throw mapStorageError(error); } this._notify(projectId, nextProject.revision, operation.kind); return clone(progress);
      });
    }

    async createPatternEvolutionProposalReview(projectId, input = {}) { const api = global.YarnAIPatternEvolutionProposalReview; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_API_MISSING", "Pattern evolution proposal review module is not loaded."); return api.createForProject(this, projectId, input); }
    async readPatternEvolutionProposalReview(projectId, reviewId = null, proposalId = null) { const api = global.YarnAIPatternEvolutionProposalReview; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_API_MISSING", "Pattern evolution proposal review module is not loaded."); return api.readForProject(this, projectId, reviewId, proposalId); }
    async revalidatePatternEvolutionProposalReview(projectId, reviewId, options = {}) { const current = await this.getPatternEvolutionProposalReview(projectId, reviewId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_NOT_FOUND", "Pattern evolution proposal review was not found."); const api = global.YarnAIPatternEvolutionProposalReview; const source = await api.loadSource(this, projectId, current.state.sourceProposalId); const next = api.revalidatePatternEvolutionProposalReview(current.state, source, { ...options, now: options.timestamp || current.state.updatedAt }); return this.savePatternEvolutionProposalReview(projectId, next, { ...options, timestamp: next.updatedAt, operationKind: "PATTERN_EVOLUTION_PROPOSAL_REVIEW_REVALIDATED", allowRevalidation: true }); }

    async _quarantinePatternEvolutionDecision(record, reasonCode) {
      const api = global.YarnAIPatternEvolutionDecision; const identity = api?.fingerprint ? api.fingerprint({ progressId: record?.progress_id || null, state: record?.state || null, reasonCode }) : canonicalize({ progressId: record?.progress_id || null, reasonCode }); const timestamp = record?.state?.updatedAt || record?.updated_at || "1970-01-01T00:00:00.000Z";
      const quarantine = { quarantine_id: `pattern-evolution-decision-quarantine:${identity}`, source_store: "progress", source_key: record?.progress_id || null, reason_code: reasonCode, record: clone(record), created_at: timestamp, expires_at: timestamp }; const database = await this._database(); const transaction = database.transaction("quarantine", "readwrite"); transaction.objectStore("quarantine").put(quarantine); await transactionComplete(transaction); return clone(quarantine);
    }

    async listPatternEvolutionDecisions(projectId, calculationId = null, filters = {}) {
      if (calculationId && typeof calculationId === "object") { filters = calculationId; calculationId = filters.calculationId || null; }
      const project = await this._validatedCurrentProject(projectId); const effectiveCalculationId = calculationId || project.active_calculation_id; if (!effectiveCalculationId) return [];
      const database = await this._database(); const transaction = database.transaction("progress", "readonly"); const records = await requestResult(transaction.objectStore("progress").index("by_calculation_kind").getAll(global.IDBKeyRange.only([effectiveCalculationId, "PATTERN_EVOLUTION_DECISION"]))); await transactionComplete(transaction);
      const filtered = records.filter((entry) => entry.project_id === projectId && (!filters.patternId || entry.state?.patternId === filters.patternId) && (!filters.proposalId && !filters.sourceProposalId || entry.state?.sourceProposalId === (filters.proposalId || filters.sourceProposalId)) && (!filters.reviewId && !filters.sourceReviewId || entry.state?.sourceReviewId === (filters.reviewId || filters.sourceReviewId)) && (!filters.lifecycle && !filters.status || entry.state?.lifecycle === (filters.lifecycle || filters.status)) && (!filters.outcome || entry.state?.outcome === filters.outcome) && (!filters.proofStatus || entry.state?.proofStatus === filters.proofStatus));
      filtered.sort((a, b) => a.epoch - b.epoch || a.state.revision - b.state.revision || (String(a.state.id) < String(b.state.id) ? -1 : String(a.state.id) > String(b.state.id) ? 1 : 0)); const valid = [];
      for (const entry of filtered) { try { validateProgressRecord(entry, projectId, effectiveCalculationId, "PATTERN_EVOLUTION_DECISION"); validateImportedPatternEvolutionDecision(entry.state, projectId); valid.push(clone(entry)); } catch (error) { await this._quarantinePatternEvolutionDecision(entry, error?.code || "INVALID_PATTERN_EVOLUTION_DECISION"); } } return valid;
    }

    async getPatternEvolutionDecision(projectId, decisionId = null, calculationId = null, reviewId = null) { const records = await this.listPatternEvolutionDecisions(projectId, calculationId, reviewId ? { reviewId } : {}); if (decisionId) return records.find((entry) => entry.progress_id === decisionId || entry.state?.id === decisionId || entry.state?.decisionId === decisionId) || null; return records.at(-1) || null; }
    async getLatestPatternEvolutionDecision(projectId, calculationId = null, filters = {}) { const records = await this.listPatternEvolutionDecisions(projectId, calculationId, filters); return records.at(-1) || null; }
    async getLatestPatternEvolutionDecisionForProposal(projectId, proposalId, calculationId = null) { return this.getLatestPatternEvolutionDecision(projectId, calculationId, { proposalId }); }
    async getLatestPatternEvolutionDecisionForReview(projectId, reviewId, calculationId = null) { return this.getLatestPatternEvolutionDecision(projectId, calculationId, { reviewId }); }

    async getPatternEvolutionDecisionSourceChain(projectId, decisionOrReviewId, calculationId = null) {
      const decision = await this.getPatternEvolutionDecision(projectId, decisionOrReviewId, calculationId); const reviewId = decision?.state?.sourceReviewId || decisionOrReviewId; const review = await this.getPatternEvolutionProposalReview(projectId, reviewId, calculationId); if (!review) return { decision, review: null, proposal: null, initiation: null };
      const proposal = await this.getPatternEvolutionProposal(projectId, review.state.sourceProposalId, review.state.calculationId); const initiation = proposal ? await this.getPatternEvolutionInitiation(projectId, proposal.state.sourceInitiationId, proposal.state.calculationId) : null; return { decision, review, proposal, initiation };
    }

    async _assertPatternEvolutionDecisionReferences(projectId, calculationId, state) {
      const api = global.YarnAIPatternEvolutionDecision; if (!api?.loadSource || !api?.calculateSourceGate) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_API_MISSING", "Pattern evolution decision module is not loaded.");
      const source = await api.loadSource(this, projectId, state.sourceReviewId, state.sourceProposalId); if (source.calculationId !== calculationId) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_SCOPE_INVALID", "Decision does not belong to the active calculation."); const gate = api.calculateSourceGate(source, state); const bindingValid = gate.reviewId === state.sourceReviewId && gate.proposalId === state.sourceProposalId && gate.initiationId === state.sourceInitiationId && gate.reviewDigest === state.sourceReviewDigest && gate.proposalDigest === state.sourceProposalDigest && gate.initiationDigest === state.sourceInitiationDigest;
      if (!bindingValid || gate.reasons.some((item) => ["source_review_revision_stale", "source_review_digest_stale", "immutable_proposal_snapshot_mismatch", "immutable_initiation_snapshot_mismatch"].includes(item.code))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_REFERENCE_INVALID", "Decision does not resolve to one immutable local review chain.", { details: { reasons: gate.reasons } }); return { source, gate };
    }

    async savePatternEvolutionDecision(projectId, calculationOrState, stateOrOptions = {}, maybeOptions = {}) {
      const directState = calculationOrState && typeof calculationOrState === "object"; const state = directState ? calculationOrState : stateOrOptions; const calculationId = directState ? state.calculationId : calculationOrState; const options = directState ? stateOrOptions : maybeOptions; const api = global.YarnAIPatternEvolutionDecision; const report = api?.validatePatternEvolutionDecision?.(state);
      if (!api || state?.kind !== "PATTERN_EVOLUTION_DECISION" || state?.type !== "PATTERN_EVOLUTION_DECISION" || state?.projectId !== projectId || state?.calculationId !== calculationId || !report?.valid) throw new ProjectRepositoryError("INVALID_PATTERN_EVOLUTION_DECISION_STATE", "Pattern evolution decision snapshot is corrupted."); validateImportedPatternEvolutionDecision(state, projectId);
      return this._serialize(projectId, async () => {
        const before = await this._validatedCurrentProject(projectId); if (before.active_calculation_id !== calculationId) throw new ProjectRepositoryError("CALCULATION_MISMATCH", "Decision does not belong to the active calculation."); const existing = await this.listPatternEvolutionDecisions(projectId, calculationId); const current = options.recordId ? existing.find((entry) => entry.progress_id === options.recordId || entry.state?.id === options.recordId) : existing.find((entry) => entry.state?.id === state.id); const revalidation = options.allowRevalidation === true && state.predecessorDecisionId && existing.some((entry) => entry.state?.id === state.predecessorDecisionId);
        if (!current) { const duplicate = api.classifyDuplicate(existing, state); if (duplicate.status === "collision") throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_COLLISION", "Decision identity collision detected."); if (["exact_duplicate", "semantic_duplicate"].includes(duplicate.status) && !revalidation) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_DUPLICATE_IDENTITY", "A decision already exists for this immutable source identity."); if (duplicate.status === "duplicate_authorized") throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_DUPLICATE_AUTHORIZED", "An authorized decision already exists for this immutable source identity."); }
        await this._assertPatternEvolutionDecisionReferences(projectId, calculationId, state); const timestamp = options.timestamp || state.updatedAt; let progress; let operationKind;
        if (current) {
          if ((options.expectedRevision !== undefined && options.expectedRevision !== current.state.revision) || (options.expectedIdentity !== undefined && options.expectedIdentity !== current.state.identity)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_REVISION_CONFLICT", "Decision was changed by another operation."); if (canonicalize(state) === canonicalize(current.state)) return clone(current); if (api.TERMINAL_LIFECYCLES.includes(current.state.lifecycle) || current.state.lifecycle === "stale") throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_TERMINAL", "Terminal and stale decisions are immutable."); if (state.id !== current.state.id || state.decisionId !== current.state.decisionId || state.epoch !== current.state.epoch || state.revision <= current.state.revision) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_REVISION_CONFLICT", "Decision revision or identity is invalid."); const allowed = { draft: ["deciding", "cancelled", "stale"], deciding: ["ready", "revision_required", "evidence_required", "blocked", "declined", "cancelled", "stale"], ready: ["authorized", "revision_required", "evidence_required", "blocked", "declined", "cancelled", "stale"] }; if (!allowed[current.state.lifecycle]?.includes(state.lifecycle)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_LIFECYCLE_CONFLICT", "Decision lifecycle transition is invalid."); const immutableFields = ["projectId", "patternId", "calculationId", "sourceInitiationId", "sourceInitiationRevision", "sourceInitiationDigest", "sourceProposalId", "sourceProposalRevision", "sourceProposalDigest", "sourceReviewId", "sourceReviewRevision", "sourceReviewDigest", "sourceSnapshots", "sourceBinding", "decisionPolicyVersion", "reviewPolicyVersion", "riskPolicyVersion", "outcome", "reasons", "conditions", "nextAction", "risk", "affectedChangeIds", "evidenceRequests", "revisionRequests", "blockers", "declineReasons", "migrationDependency", "rollbackObligations", "compatibilityRestrictions", "predecessorDecisionId", "supersedesDecisionId", "epoch", "createdAt", "identity"]; if (immutableFields.some((field) => canonicalize(state[field]) !== canonicalize(current.state[field]))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_IMMUTABLE", "Decision semantics and source snapshots are immutable."); progress = clone(current); progress.state = clone(state); progress.revision += 1; progress.updated_at = timestamp; operationKind = options.operationKind || `PATTERN_EVOLUTION_DECISION_${state.lifecycle.toUpperCase()}`;
        } else { progress = this._initialProgress(projectId, calculationId, "PATTERN_EVOLUTION_DECISION", timestamp); progress.epoch = existing.reduce((maximum, entry) => Math.max(maximum, entry.epoch), 0) + 1; progress.state = clone(state); if (progress.state.epoch !== progress.epoch) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_EPOCH_CONFLICT", "Decision epoch differs from progress epoch."); operationKind = options.operationKind || (revalidation ? "PATTERN_EVOLUTION_DECISION_REVALIDATED" : "PATTERN_EVOLUTION_DECISION_CREATED"); }
        const nextProject = clone(before); nextProject.current_stage = `pattern_evolution_decision_${state.lifecycle}`; nextProject.updated_at = timestamp; nextProject.revision += 1; nextProject.materialized_checksum = await checksumPayload(projectChecksumPayload(nextProject)); const operation = createOperation(nextProject, operationKind, { calculation_id: calculationId, progress_id: progress.progress_id, progress_epoch: progress.epoch, progress_revision: progress.revision, progress_state: clone(state) }, timestamp, before.revision, nextProject.revision); const checkpoint = createCheckpoint(nextProject, nextProject.materialized_checksum, nextProject.revision, timestamp);
        const database = await this._database(); const transaction = database.transaction(["projects", "progress", "operations", "checkpoints", "meta"], "readwrite"); try { const storedProject = await requestResult(transaction.objectStore("projects").get(projectId)); const storedProgress = current ? await requestResult(transaction.objectStore("progress").get(current.progress_id)) : null; if (!storedProject || storedProject.revision !== before.revision || current && (!storedProgress || storedProgress.revision !== current.revision)) { transaction.abort(); throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_REVISION_CONFLICT", "Decision changed concurrently."); } await allocateOperationMetadata(transaction, operation); current ? transaction.objectStore("progress").put(progress) : transaction.objectStore("progress").add(progress); transaction.objectStore("projects").put(nextProject); transaction.objectStore("operations").add(operation); transaction.objectStore("checkpoints").add(checkpoint); await transactionComplete(transaction); } catch (error) { throw mapStorageError(error); } this._notify(projectId, nextProject.revision, operation.kind); return clone(progress);
      });
    }

    async transitionPatternEvolutionDecision(projectId, decisionId, nextLifecycle, options = {}) { const current = await this.getPatternEvolutionDecision(projectId, decisionId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_NOT_FOUND", "Pattern evolution decision was not found."); const api = global.YarnAIPatternEvolutionDecision; const source = await api.loadSource(this, projectId, current.state.sourceReviewId, current.state.sourceProposalId); const command = { now: options.timestamp || current.state.updatedAt, expectedRevision: options.expectedRevision ?? current.state.revision, expectedIdentity: options.expectedIdentity ?? current.state.identity }; const next = nextLifecycle === "deciding" ? api.startDecision(current.state, source, command) : nextLifecycle === "ready" ? api.markDecisionReady(current.state, source, command) : nextLifecycle === "cancelled" ? api.cancelDecision(current.state, source, command) : api.finalizeDecision(current.state, source, command); return this.savePatternEvolutionDecision(projectId, next, { recordId: current.progress_id, expectedRevision: current.state.revision, expectedIdentity: current.state.identity, timestamp: next.updatedAt }); }
    async createPatternEvolutionDecision(projectId, input = {}) { const api = global.YarnAIPatternEvolutionDecision; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_API_MISSING", "Pattern evolution decision module is not loaded."); return api.createForProject(this, projectId, input); }
    async readPatternEvolutionDecision(projectId, decisionId = null, reviewId = null, proposalId = null) { const api = global.YarnAIPatternEvolutionDecision; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_API_MISSING", "Pattern evolution decision module is not loaded."); return api.readForProject(this, projectId, decisionId, reviewId, proposalId); }
    async revalidatePatternEvolutionDecision(projectId, decisionId, options = {}) { const current = await this.getPatternEvolutionDecision(projectId, decisionId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_NOT_FOUND", "Pattern evolution decision was not found."); const api = global.YarnAIPatternEvolutionDecision; const source = await api.loadSource(this, projectId, current.state.sourceReviewId, current.state.sourceProposalId); const next = api.revalidatePatternEvolutionDecision(current.state, source, { ...options, now: options.timestamp || current.state.updatedAt }); return this.savePatternEvolutionDecision(projectId, next, { ...options, timestamp: next.updatedAt, operationKind: "PATTERN_EVOLUTION_DECISION_REVALIDATED", allowRevalidation: true }); }
    async exportPatternEvolutionDecisions(projectId, calculationId = null) { const api = global.YarnAIPatternEvolutionDecision; const records = await this.listPatternEvolutionDecisions(projectId, calculationId); return records.map((entry) => api.serializePatternEvolutionDecision(entry.state)); }
    async importPatternEvolutionDecision(projectId, serialized, options = {}) { const api = global.YarnAIPatternEvolutionDecision; const existing = await this.listPatternEvolutionDecisions(projectId, options.calculationId || null); const result = api.importPatternEvolutionDecision(existing, serialized, options); if (result.status === "malformed") { await this._quarantinePatternEvolutionDecision({ progress_id: null, state: serialized, updated_at: options.timestamp }, result.quarantine?.reasonCode || "INVALID_PATTERN_EVOLUTION_DECISION_IMPORT"); return result; } if (!result.changed) return result; const stored = await this.savePatternEvolutionDecision(projectId, result.record, { timestamp: options.timestamp || result.record.updatedAt, allowImported: true }); return { ...result, stored }; }

    async _quarantinePatternEvolutionExecution(record, reasonCode) {
      const api = global.YarnAIPatternEvolutionExecution; const identity = api?.fingerprint ? api.fingerprint({ progressId: record?.progress_id || null, state: record?.state || null, reasonCode }) : canonicalize({ progressId: record?.progress_id || null, reasonCode }); const timestamp = record?.state?.updatedAt || record?.updated_at || "1970-01-01T00:00:00.000Z";
      const quarantine = { quarantine_id: `pattern-evolution-execution-quarantine:${identity}`, source_store: "progress", source_key: record?.progress_id || null, reason_code: reasonCode, record: clone(record), created_at: timestamp, expires_at: timestamp }; const database = await this._database(); const transaction = database.transaction("quarantine", "readwrite"); transaction.objectStore("quarantine").put(quarantine); await transactionComplete(transaction); return clone(quarantine);
    }

    async listPatternEvolutionExecutions(projectId, calculationId = null, filters = {}) {
      if (calculationId && typeof calculationId === "object") { filters = calculationId; calculationId = filters.calculationId || null; }
      const project = await this._validatedCurrentProject(projectId); const effectiveCalculationId = calculationId || project.active_calculation_id; if (!effectiveCalculationId) return [];
      const database = await this._database(); const transaction = database.transaction("progress", "readonly"); const records = await requestResult(transaction.objectStore("progress").index("by_calculation_kind").getAll(global.IDBKeyRange.only([effectiveCalculationId, "PATTERN_EVOLUTION_EXECUTION"]))); await transactionComplete(transaction);
      const filtered = records.filter((entry) => entry.project_id === projectId && (!filters.patternId || entry.state?.patternId === filters.patternId) && (!filters.decisionId && !filters.sourceDecisionId || entry.state?.sourceDecisionId === (filters.decisionId || filters.sourceDecisionId)) && (!filters.proposalId || entry.state?.sourceProposalId === filters.proposalId) && (!filters.reviewId || entry.state?.sourceReviewId === filters.reviewId) && (!filters.lifecycle || entry.state?.lifecycle === filters.lifecycle) && (!filters.status || entry.state?.status === filters.status) && (filters.importedUnproven === undefined || entry.state?.importedUnproven === filters.importedUnproven));
      filtered.sort((a, b) => a.epoch - b.epoch || a.state.revision - b.state.revision || (String(a.state.id) < String(b.state.id) ? -1 : String(a.state.id) > String(b.state.id) ? 1 : 0)); const valid = [];
      for (const entry of filtered) { try { validateProgressRecord(entry, projectId, effectiveCalculationId, "PATTERN_EVOLUTION_EXECUTION"); validateImportedPatternEvolutionExecution(entry.state, projectId); valid.push(clone(entry)); } catch (error) { await this._quarantinePatternEvolutionExecution(entry, error?.code || "INVALID_PATTERN_EVOLUTION_EXECUTION"); } } return valid;
    }

    async getPatternEvolutionExecution(projectId, executionId = null, calculationId = null, decisionId = null) { const records = await this.listPatternEvolutionExecutions(projectId, calculationId, decisionId ? { decisionId } : {}); if (executionId) return records.find((entry) => entry.progress_id === executionId || entry.state?.id === executionId || entry.state?.executionId === executionId) || null; return records.at(-1) || null; }
    async getLatestPatternEvolutionExecution(projectId, calculationId = null, filters = {}) { const records = await this.listPatternEvolutionExecutions(projectId, calculationId, filters); return records.at(-1) || null; }
    async getLatestPatternEvolutionExecutionForDecision(projectId, decisionId, calculationId = null) { return this.getLatestPatternEvolutionExecution(projectId, calculationId, { decisionId }); }
    async getLatestPatternEvolutionExecutionForProposal(projectId, proposalId, calculationId = null) { return this.getLatestPatternEvolutionExecution(projectId, calculationId, { proposalId }); }
    async getLatestPatternEvolutionExecutionForReview(projectId, reviewId, calculationId = null) { return this.getLatestPatternEvolutionExecution(projectId, calculationId, { reviewId }); }
    async getLatestPatternEvolutionExecutionForPattern(projectId, patternId, calculationId = null) { return this.getLatestPatternEvolutionExecution(projectId, calculationId, { patternId }); }

    async getPatternEvolutionExecutionSourceChain(projectId, executionOrDecisionId, calculationId = null) {
      const execution = await this.getPatternEvolutionExecution(projectId, executionOrDecisionId, calculationId); const decisionId = execution?.state?.sourceDecisionId || executionOrDecisionId; const decision = await this.getPatternEvolutionDecision(projectId, decisionId, calculationId); if (!decision) return { execution, decision: null, review: null, proposal: null, initiation: null }; const chain = await this.getPatternEvolutionDecisionSourceChain(projectId, decision.state.id, decision.state.calculationId); return { execution, decision, ...chain };
    }

    async _assertPatternEvolutionExecutionReferences(projectId, calculationId, state) {
      const api = global.YarnAIPatternEvolutionExecution; if (!api?.loadSource || !api?.calculateSourceGate) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_API_MISSING", "Pattern evolution execution module is not loaded."); const source = await api.loadSource(this, projectId, state.sourceDecisionId); if (source.calculationId !== calculationId) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_SCOPE_INVALID", "Execution does not belong to the active calculation."); const gate = api.calculateSourceGate(source, state); if (!gate.valid) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_REFERENCE_INVALID", "Execution does not resolve to one immutable authorized local decision chain.", { details: { reasons: gate.reasons } }); return { source, gate };
    }

    async savePatternEvolutionExecution(projectId, calculationOrState, stateOrOptions = {}, maybeOptions = {}) {
      const directState = calculationOrState && typeof calculationOrState === "object"; const state = directState ? calculationOrState : stateOrOptions; const calculationId = directState ? state.calculationId : calculationOrState; const options = directState ? stateOrOptions : maybeOptions; const api = global.YarnAIPatternEvolutionExecution; const report = api?.validatePatternEvolutionExecution?.(state);
      if (!api || state?.kind !== "PATTERN_EVOLUTION_EXECUTION" || state?.type !== "PATTERN_EVOLUTION_EXECUTION" || state?.projectId !== projectId || state?.calculationId !== calculationId || !report?.valid) throw new ProjectRepositoryError("INVALID_PATTERN_EVOLUTION_EXECUTION_STATE", "Pattern evolution execution snapshot is corrupted."); validateImportedPatternEvolutionExecution(state, projectId);
      return this._serialize(projectId, async () => {
        const before = await this._validatedCurrentProject(projectId); if (before.active_calculation_id !== calculationId) throw new ProjectRepositoryError("CALCULATION_MISMATCH", "Execution does not belong to the active calculation."); const existing = await this.listPatternEvolutionExecutions(projectId, calculationId); const current = options.recordId ? existing.find((entry) => entry.progress_id === options.recordId || entry.state?.id === options.recordId) : existing.find((entry) => entry.state?.id === state.id); const revalidation = options.allowRevalidation === true && state.predecessorExecutionId && existing.some((entry) => entry.state?.id === state.predecessorExecutionId);
        if (!current) { const duplicate = api.classifyDuplicate(existing, state); if (["collision", "identity_collision"].includes(duplicate.status)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_COLLISION", "Execution identity collision detected."); if (duplicate.status === "duplicate_completed") throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_DUPLICATE_COMPLETED", "A completed execution already proves this semantic envelope."); if (duplicate.status === "semantic_duplicate" && !revalidation) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_SEMANTIC_DUPLICATE", "An execution already exists for this semantic envelope."); }
        await this._assertPatternEvolutionExecutionReferences(projectId, calculationId, state); const timestamp = options.timestamp || state.updatedAt; let progress; let operationKind;
        if (current) {
          if ((options.expectedRevision !== undefined && options.expectedRevision !== current.state.revision) || (options.expectedIdentity !== undefined && options.expectedIdentity !== current.state.identity)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_REVISION_CONFLICT", "Execution was changed by another operation."); if (canonicalize(state) === canonicalize(current.state)) return clone(current); if (api.TERMINAL_LIFECYCLES.includes(current.state.lifecycle) || current.state.lifecycle === "stale") throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_TERMINAL", "Terminal and stale executions are immutable."); if (state.id !== current.state.id || state.executionId !== current.state.executionId || state.epoch !== current.state.epoch || state.revision <= current.state.revision) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_REVISION_CONFLICT", "Execution revision or identity is invalid."); if (!api.TRANSITIONS[current.state.lifecycle]?.includes(state.lifecycle) && state.lifecycle !== current.state.lifecycle) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_LIFECYCLE_CONFLICT", "Execution lifecycle transition is invalid."); const immutableFields = ["projectId", "patternId", "calculationId", "sourceInitiationId", "sourceInitiationRevision", "sourceInitiationDigest", "sourceProposalId", "sourceProposalRevision", "sourceProposalDigest", "sourceReviewId", "sourceReviewRevision", "sourceReviewDigest", "sourceDecisionId", "sourceDecisionRevision", "sourceDecisionDigest", "sourceChainDigest", "sourceSnapshots", "sourceEnvelope", "executionPlan", "risk", "executionPolicyVersion", "riskPolicyVersion", "evidencePolicyVersion", "identity", "semanticIdentity", "predecessorExecutionId", "supersedesExecutionId", "epoch", "createdAt"]; if (immutableFields.some((field) => canonicalize(state[field]) !== canonicalize(current.state[field]))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_IMMUTABLE", "Execution semantics and source snapshots are immutable."); progress = clone(current); progress.state = clone(state); progress.revision += 1; progress.updated_at = timestamp; operationKind = options.operationKind || `PATTERN_EVOLUTION_EXECUTION_${state.lifecycle.toUpperCase()}`;
        } else { progress = this._initialProgress(projectId, calculationId, "PATTERN_EVOLUTION_EXECUTION", timestamp); progress.epoch = existing.reduce((maximum, entry) => Math.max(maximum, entry.epoch), 0) + 1; progress.state = clone(state); if (progress.state.epoch !== progress.epoch) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_EPOCH_CONFLICT", "Execution epoch differs from progress epoch."); operationKind = options.operationKind || (revalidation ? "PATTERN_EVOLUTION_EXECUTION_REVALIDATED" : "PATTERN_EVOLUTION_EXECUTION_CREATED"); }
        const nextProject = clone(before); nextProject.current_stage = `pattern_evolution_execution_${state.lifecycle}`; nextProject.updated_at = timestamp; nextProject.revision += 1; nextProject.materialized_checksum = await checksumPayload(projectChecksumPayload(nextProject)); const operation = createOperation(nextProject, operationKind, { calculation_id: calculationId, progress_id: progress.progress_id, progress_epoch: progress.epoch, progress_revision: progress.revision, progress_state: clone(state) }, timestamp, before.revision, nextProject.revision); const checkpoint = createCheckpoint(nextProject, nextProject.materialized_checksum, nextProject.revision, timestamp);
        const database = await this._database(); const transaction = database.transaction(["projects", "progress", "operations", "checkpoints", "meta"], "readwrite"); try { const storedProject = await requestResult(transaction.objectStore("projects").get(projectId)); const storedProgress = current ? await requestResult(transaction.objectStore("progress").get(current.progress_id)) : null; if (!storedProject || storedProject.revision !== before.revision || current && (!storedProgress || storedProgress.revision !== current.revision)) { transaction.abort(); throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_REVISION_CONFLICT", "Execution changed concurrently."); } await allocateOperationMetadata(transaction, operation); current ? transaction.objectStore("progress").put(progress) : transaction.objectStore("progress").add(progress); transaction.objectStore("projects").put(nextProject); transaction.objectStore("operations").add(operation); transaction.objectStore("checkpoints").add(checkpoint); await transactionComplete(transaction); } catch (error) { throw mapStorageError(error); } this._notify(projectId, nextProject.revision, operation.kind); return clone(progress);
      });
    }

    async transitionPatternEvolutionExecution(projectId, executionId, nextLifecycle, options = {}) { const current = await this.getPatternEvolutionExecution(projectId, executionId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_NOT_FOUND", "Pattern evolution execution was not found."); const api = global.YarnAIPatternEvolutionExecution; const source = await api.loadSource(this, projectId, current.state.sourceDecisionId); const command = { now: options.timestamp || current.state.updatedAt, expectedRevision: options.expectedRevision ?? current.state.revision, expectedIdentity: options.expectedIdentity ?? current.state.identity }; const commands = { preparing: api.prepareExecution, ready: api.markExecutionReady, executing: api.startExecution, verifying: api.beginVerification, completed: api.completeExecution, cancelled: api.cancelExecution, stale: api.markExecutionStale, revision_required: api.requireExecutionRevision, evidence_required: api.requireExecutionEvidence, rollback_required: api.requireExecutionRollback, blocked: api.blockExecution, failed: api.failExecution }; const transition = commands[nextLifecycle]; if (!transition) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_LIFECYCLE_CONFLICT", "Execution lifecycle transition is unsupported."); const next = transition(current.state, source, command); return this.savePatternEvolutionExecution(projectId, next, { recordId: current.progress_id, expectedRevision: current.state.revision, expectedIdentity: current.state.identity, timestamp: next.updatedAt }); }
    async recordPatternEvolutionExecutionObservation(projectId, executionId, observation, evidence = [], options = {}) { const current = await this.getPatternEvolutionExecution(projectId, executionId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_NOT_FOUND", "Pattern evolution execution was not found."); const api = global.YarnAIPatternEvolutionExecution; const source = await api.loadSource(this, projectId, current.state.sourceDecisionId); const next = api.recordExecutionObservation(current.state, observation, evidence, source, { now: options.timestamp || current.state.updatedAt, expectedRevision: options.expectedRevision ?? current.state.revision, expectedIdentity: options.expectedIdentity ?? current.state.identity }); return this.savePatternEvolutionExecution(projectId, next, { recordId: current.progress_id, expectedRevision: current.state.revision, expectedIdentity: current.state.identity, timestamp: next.updatedAt, operationKind: "PATTERN_EVOLUTION_EXECUTION_OBSERVATION_RECORDED" }); }
    async createPatternEvolutionExecution(projectId, input = {}) { const api = global.YarnAIPatternEvolutionExecution; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_API_MISSING", "Pattern evolution execution module is not loaded."); return api.createForProject(this, projectId, input); }
    async readPatternEvolutionExecution(projectId, executionId = null, decisionId = null) { const api = global.YarnAIPatternEvolutionExecution; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_API_MISSING", "Pattern evolution execution module is not loaded."); return api.readForProject(this, projectId, executionId, decisionId); }
    async revalidatePatternEvolutionExecution(projectId, executionId, options = {}) { const current = await this.getPatternEvolutionExecution(projectId, executionId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_NOT_FOUND", "Pattern evolution execution was not found."); const api = global.YarnAIPatternEvolutionExecution; const source = await api.loadSource(this, projectId, current.state.sourceDecisionId); const next = api.revalidatePatternEvolutionExecution(current.state, source, { ...options, now: options.timestamp || current.state.updatedAt }); return this.savePatternEvolutionExecution(projectId, next, { ...options, timestamp: next.updatedAt, operationKind: "PATTERN_EVOLUTION_EXECUTION_REVALIDATED", allowRevalidation: true }); }
    async exportPatternEvolutionExecutions(projectId, calculationId = null) { const api = global.YarnAIPatternEvolutionExecution; const records = await this.listPatternEvolutionExecutions(projectId, calculationId); return records.map((entry) => api.serializePatternEvolutionExecution(entry.state)); }
    async importPatternEvolutionExecution(projectId, serialized, options = {}) { const api = global.YarnAIPatternEvolutionExecution; const existing = await this.listPatternEvolutionExecutions(projectId, options.calculationId || null); const result = api.importPatternEvolutionExecution(existing, serialized, options); if (result.status === "malformed") { await this._quarantinePatternEvolutionExecution({ progress_id: null, state: serialized, updated_at: options.timestamp }, result.quarantine?.reasonCode || "INVALID_PATTERN_EVOLUTION_EXECUTION_IMPORT"); return result; } if (!result.changed) return result; const stored = await this.savePatternEvolutionExecution(projectId, result.record, { timestamp: options.timestamp || result.record.updatedAt, allowImported: true }); return { ...result, stored }; }

    async _quarantinePatternEvolutionExecutionVerification(record, reasonCode) {
      const api = global.YarnAIPatternEvolutionExecutionVerification; const identity = api?.fingerprint ? api.fingerprint({ progressId: record?.progress_id || null, state: record?.state || null, reasonCode }) : canonicalize({ progressId: record?.progress_id || null, reasonCode }); const timestamp = record?.state?.updatedAt || record?.updated_at || "1970-01-01T00:00:00.000Z";
      const quarantine = { quarantine_id: `pattern-evolution-execution-verification-quarantine:${identity}`, source_store: "progress", source_key: record?.progress_id || null, reason_code: reasonCode, record: clone(record), created_at: timestamp, expires_at: timestamp }; const database = await this._database(); const transaction = database.transaction("quarantine", "readwrite"); transaction.objectStore("quarantine").put(quarantine); await transactionComplete(transaction); return clone(quarantine);
    }

    async listPatternEvolutionExecutionVerifications(projectId, calculationId = null, filters = {}) {
      if (calculationId && typeof calculationId === "object") { filters = calculationId; calculationId = filters.calculationId || null; }
      const project = await this._validatedCurrentProject(projectId); const effectiveCalculationId = calculationId || project.active_calculation_id; if (!effectiveCalculationId) return [];
      const database = await this._database(); const transaction = database.transaction("progress", "readonly"); const records = await requestResult(transaction.objectStore("progress").index("by_calculation_kind").getAll(global.IDBKeyRange.only([effectiveCalculationId, "PATTERN_EVOLUTION_EXECUTION_VERIFICATION"]))); await transactionComplete(transaction);
      const filtered = records.filter((entry) => entry.project_id === projectId && (!filters.patternId || entry.state?.patternId === filters.patternId) && (!filters.executionId && !filters.sourceExecutionId || entry.state?.sourceExecutionId === (filters.executionId || filters.sourceExecutionId)) && (!filters.decisionId || entry.state?.sourceDecisionId === filters.decisionId) && (!filters.lifecycle || entry.state?.lifecycle === filters.lifecycle) && (!filters.status || entry.state?.status === filters.status) && (!filters.verdict || entry.state?.verdict === filters.verdict) && (filters.importedUnproven === undefined || entry.state?.importedUnproven === filters.importedUnproven));
      filtered.sort((a, b) => a.epoch - b.epoch || a.state.revision - b.state.revision || (String(a.state.id) < String(b.state.id) ? -1 : String(a.state.id) > String(b.state.id) ? 1 : 0)); const valid = [];
      for (const entry of filtered) { try { validateProgressRecord(entry, projectId, effectiveCalculationId, "PATTERN_EVOLUTION_EXECUTION_VERIFICATION"); validateImportedPatternEvolutionExecutionVerification(entry.state, projectId); valid.push(clone(entry)); } catch (error) { await this._quarantinePatternEvolutionExecutionVerification(entry, error?.code || "INVALID_PATTERN_EVOLUTION_EXECUTION_VERIFICATION"); } } return valid;
    }

    async getPatternEvolutionExecutionVerification(projectId, verificationId = null, calculationId = null, executionId = null) { const records = await this.listPatternEvolutionExecutionVerifications(projectId, calculationId, executionId ? { executionId } : {}); if (verificationId) return records.find((entry) => entry.progress_id === verificationId || entry.state?.id === verificationId || entry.state?.verificationId === verificationId) || null; return records.at(-1) || null; }
    async getLatestPatternEvolutionExecutionVerification(projectId, calculationId = null, filters = {}) { const records = await this.listPatternEvolutionExecutionVerifications(projectId, calculationId, filters); const superseded = new Set(records.flatMap((entry) => [entry.state?.predecessorVerificationId, entry.state?.supersedesVerificationId]).filter(Boolean)); return records.filter((entry) => !superseded.has(entry.state?.id) && entry.state?.lifecycle !== "stale" && !entry.state?.importedUnproven && !entry.state?.collision && !entry.state?.quarantined).at(-1) || null; }
    async getLatestPatternEvolutionExecutionVerificationForExecution(projectId, executionId, calculationId = null) { return this.getLatestPatternEvolutionExecutionVerification(projectId, calculationId, { executionId }); }
    async getLatestPatternEvolutionExecutionVerificationForDecision(projectId, decisionId, calculationId = null) { return this.getLatestPatternEvolutionExecutionVerification(projectId, calculationId, { decisionId }); }
    async getLatestPatternEvolutionExecutionVerificationForPattern(projectId, patternId, calculationId = null) { return this.getLatestPatternEvolutionExecutionVerification(projectId, calculationId, { patternId }); }

    async getPatternEvolutionExecutionVerificationSourceChain(projectId, verificationOrExecutionId, calculationId = null) {
      const verification = await this.getPatternEvolutionExecutionVerification(projectId, verificationOrExecutionId, calculationId); const executionId = verification?.state?.sourceExecutionId || verificationOrExecutionId; const execution = await this.getPatternEvolutionExecution(projectId, executionId, calculationId); if (!execution) return { verification, execution: null, decision: null, review: null, proposal: null, initiation: null }; const chain = await this.getPatternEvolutionExecutionSourceChain(projectId, execution.state.id, execution.state.calculationId); return { verification, execution, decision: chain.decision, review: chain.review, proposal: chain.proposal, initiation: chain.initiation };
    }

    async _assertPatternEvolutionExecutionVerificationReferences(projectId, calculationId, state) {
      const api = global.YarnAIPatternEvolutionExecutionVerification; if (!api?.loadSource || !api?.calculateSourceGate) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_API_MISSING", "Pattern evolution execution verification module is not loaded."); const source = await api.loadSource(this, projectId, state.sourceExecutionId); if (source.calculationId !== calculationId) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_SCOPE_INVALID", "Verification does not belong to the active calculation."); const gate = api.calculateSourceGate(source, state); if (!gate.valid) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_REFERENCE_INVALID", "Verification does not resolve to one immutable terminal local execution chain.", { details: { reasons: gate.reasons } }); return { source, gate };
    }

    async savePatternEvolutionExecutionVerification(projectId, calculationOrState, stateOrOptions = {}, maybeOptions = {}) {
      const directState = calculationOrState && typeof calculationOrState === "object"; const state = directState ? calculationOrState : stateOrOptions; const calculationId = directState ? state.calculationId : calculationOrState; const options = directState ? stateOrOptions : maybeOptions; const api = global.YarnAIPatternEvolutionExecutionVerification; const report = api?.validatePatternEvolutionExecutionVerification?.(state);
      if (!api || state?.kind !== "PATTERN_EVOLUTION_EXECUTION_VERIFICATION" || state?.type !== "PATTERN_EVOLUTION_EXECUTION_VERIFICATION" || state?.projectId !== projectId || state?.calculationId !== calculationId || !report?.valid) throw new ProjectRepositoryError("INVALID_PATTERN_EVOLUTION_EXECUTION_VERIFICATION_STATE", "Pattern evolution execution verification snapshot is corrupted."); validateImportedPatternEvolutionExecutionVerification(state, projectId);
      return this._serialize(projectId, async () => {
        const before = await this._validatedCurrentProject(projectId); if (before.active_calculation_id !== calculationId) throw new ProjectRepositoryError("CALCULATION_MISMATCH", "Verification does not belong to the active calculation."); const existing = await this.listPatternEvolutionExecutionVerifications(projectId, calculationId); const current = options.recordId ? existing.find((entry) => entry.progress_id === options.recordId || entry.state?.id === options.recordId) : existing.find((entry) => entry.state?.id === state.id); const revalidation = options.allowRevalidation === true && state.predecessorVerificationId && existing.some((entry) => entry.state?.id === state.predecessorVerificationId);
        if (!current) { const duplicate = api.classifyDuplicate(existing, state); if (["collision", "identity_collision"].includes(duplicate.status)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_COLLISION", "Verification identity collision detected."); if (duplicate.status === "duplicate_completed") throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_DUPLICATE_COMPLETED", "A completed verification already proves this semantic envelope."); if (duplicate.status === "semantic_duplicate" && !revalidation) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_SEMANTIC_DUPLICATE", "A verification already exists for this semantic envelope."); }
        if (!(options.allowImported === true && state.importedUnproven)) await this._assertPatternEvolutionExecutionVerificationReferences(projectId, calculationId, state); const timestamp = options.timestamp || state.updatedAt; let progress; let operationKind;
        if (current) {
          if ((options.expectedRevision !== undefined && options.expectedRevision !== current.state.revision) || (options.expectedIdentity !== undefined && options.expectedIdentity !== current.state.identity)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_REVISION_CONFLICT", "Verification was changed by another operation."); if (canonicalize(state) === canonicalize(current.state)) return clone(current); if (api.TERMINAL_LIFECYCLES.includes(current.state.lifecycle) || current.state.lifecycle === "stale") throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_TERMINAL", "Terminal and stale verifications are immutable."); if (state.id !== current.state.id || state.verificationId !== current.state.verificationId || state.epoch !== current.state.epoch || state.revision <= current.state.revision) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_REVISION_CONFLICT", "Verification revision or identity is invalid."); if (!api.TRANSITIONS[current.state.lifecycle]?.includes(state.lifecycle) && state.lifecycle !== current.state.lifecycle) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_LIFECYCLE_CONFLICT", "Verification lifecycle transition is invalid."); const immutableFields = ["projectId", "patternId", "calculationId", "sourceInitiationId", "sourceProposalId", "sourceReviewId", "sourceDecisionId", "sourceExecutionId", "sourceExecutionRevision", "sourceExecutionDigest", "sourceChainDigest", "sourceSnapshots", "sourceBinding", "verificationScope", "risk", "verificationPolicyVersion", "evidencePolicyVersion", "identity", "semanticIdentity", "predecessorVerificationId", "supersedesVerificationId", "epoch", "createdAt"]; if (immutableFields.some((field) => canonicalize(state[field]) !== canonicalize(current.state[field]))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_IMMUTABLE", "Verification semantics and source snapshots are immutable."); progress = clone(current); progress.state = clone(state); progress.revision += 1; progress.updated_at = timestamp; operationKind = options.operationKind || `PATTERN_EVOLUTION_EXECUTION_VERIFICATION_${state.lifecycle.toUpperCase()}`;
        } else { progress = this._initialProgress(projectId, calculationId, "PATTERN_EVOLUTION_EXECUTION_VERIFICATION", timestamp); progress.epoch = existing.reduce((maximum, entry) => Math.max(maximum, entry.epoch), 0) + 1; progress.state = clone(state); if (progress.state.epoch !== progress.epoch) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_EPOCH_CONFLICT", "Verification epoch differs from progress epoch."); operationKind = options.operationKind || (revalidation ? "PATTERN_EVOLUTION_EXECUTION_VERIFICATION_REVALIDATED" : state.importedUnproven ? "PATTERN_EVOLUTION_EXECUTION_VERIFICATION_IMPORTED" : "PATTERN_EVOLUTION_EXECUTION_VERIFICATION_CREATED"); }
        const nextProject = clone(before); nextProject.current_stage = `pattern_evolution_execution_verification_${state.lifecycle}`; nextProject.updated_at = timestamp; nextProject.revision += 1; nextProject.materialized_checksum = await checksumPayload(projectChecksumPayload(nextProject)); const operation = createOperation(nextProject, operationKind, { calculation_id: calculationId, progress_id: progress.progress_id, progress_epoch: progress.epoch, progress_revision: progress.revision, progress_state: clone(state) }, timestamp, before.revision, nextProject.revision); const checkpoint = createCheckpoint(nextProject, nextProject.materialized_checksum, nextProject.revision, timestamp);
        const database = await this._database(); const transaction = database.transaction(["projects", "progress", "operations", "checkpoints", "meta"], "readwrite"); try { const storedProject = await requestResult(transaction.objectStore("projects").get(projectId)); const storedProgress = current ? await requestResult(transaction.objectStore("progress").get(current.progress_id)) : null; if (!storedProject || storedProject.revision !== before.revision || current && (!storedProgress || storedProgress.revision !== current.revision)) { transaction.abort(); throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_REVISION_CONFLICT", "Verification changed concurrently."); } await allocateOperationMetadata(transaction, operation); current ? transaction.objectStore("progress").put(progress) : transaction.objectStore("progress").add(progress); transaction.objectStore("projects").put(nextProject); transaction.objectStore("operations").add(operation); transaction.objectStore("checkpoints").add(checkpoint); await transactionComplete(transaction); } catch (error) { throw mapStorageError(error); } this._notify(projectId, nextProject.revision, operation.kind); return clone(progress);
      });
    }

    async transitionPatternEvolutionExecutionVerification(projectId, verificationId, nextLifecycle, options = {}) { const current = await this.getPatternEvolutionExecutionVerification(projectId, verificationId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_NOT_FOUND", "Pattern evolution execution verification was not found."); const api = global.YarnAIPatternEvolutionExecutionVerification; const source = await api.loadSource(this, projectId, current.state.sourceExecutionId); const command = { now: options.timestamp || current.state.updatedAt, expectedRevision: options.expectedRevision ?? current.state.revision, expectedIdentity: options.expectedIdentity ?? current.state.identity }; const commands = { collecting: api.startCollecting, evaluating: api.startEvaluating, reviewing: api.startReviewing, stale: api.markVerificationStale }; let transition = commands[nextLifecycle]; let next; if (api.TERMINAL_LIFECYCLES.includes(nextLifecycle)) { next = api.finalizeVerification(current.state, source, command); if (next.lifecycle !== nextLifecycle) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_LIFECYCLE_CONFLICT", "Requested terminal lifecycle differs from the computed verdict."); } else { if (!transition) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_LIFECYCLE_CONFLICT", "Verification lifecycle transition is unsupported."); next = transition(current.state, source, command); } return this.savePatternEvolutionExecutionVerification(projectId, next, { recordId: current.progress_id, expectedRevision: current.state.revision, expectedIdentity: current.state.identity, timestamp: next.updatedAt }); }
    async createPatternEvolutionExecutionVerification(projectId, input = {}) { const api = global.YarnAIPatternEvolutionExecutionVerification; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_API_MISSING", "Pattern evolution execution verification module is not loaded."); return api.createForProject(this, projectId, input); }
    async readPatternEvolutionExecutionVerification(projectId, verificationId = null, executionId = null) { const api = global.YarnAIPatternEvolutionExecutionVerification; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_API_MISSING", "Pattern evolution execution verification module is not loaded."); return api.readForProject(this, projectId, verificationId, executionId); }
    async revalidatePatternEvolutionExecutionVerification(projectId, verificationId, options = {}) { const current = await this.getPatternEvolutionExecutionVerification(projectId, verificationId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_NOT_FOUND", "Pattern evolution execution verification was not found."); const api = global.YarnAIPatternEvolutionExecutionVerification; const source = await api.loadSource(this, projectId, current.state.sourceExecutionId); const next = api.revalidatePatternEvolutionExecutionVerification(current.state, source, { ...options, now: options.timestamp || current.state.updatedAt }); return this.savePatternEvolutionExecutionVerification(projectId, next, { ...options, timestamp: next.updatedAt, operationKind: "PATTERN_EVOLUTION_EXECUTION_VERIFICATION_REVALIDATED", allowRevalidation: true }); }
    async exportPatternEvolutionExecutionVerifications(projectId, calculationId = null) { const api = global.YarnAIPatternEvolutionExecutionVerification; const records = await this.listPatternEvolutionExecutionVerifications(projectId, calculationId); return records.map((entry) => api.serializePatternEvolutionExecutionVerification(entry.state)); }
    async importPatternEvolutionExecutionVerification(projectId, serialized, options = {}) { const api = global.YarnAIPatternEvolutionExecutionVerification; const existing = await this.listPatternEvolutionExecutionVerifications(projectId, options.calculationId || null); const result = api.importPatternEvolutionExecutionVerification(existing, serialized, options); if (["malformed", "collision"].includes(result.status)) { await this._quarantinePatternEvolutionExecutionVerification({ progress_id: null, state: serialized, updated_at: options.timestamp }, result.quarantine?.reasonCode || "INVALID_PATTERN_EVOLUTION_EXECUTION_VERIFICATION_IMPORT"); return result; } if (!result.changed) return result; const stored = await this.savePatternEvolutionExecutionVerification(projectId, result.record, { timestamp: options.timestamp || result.record.updatedAt, allowImported: true }); return { ...result, stored }; }

    async _quarantinePatternEvolutionAcceptance(record, reasonCode) {
      const api = global.YarnAIPatternEvolutionAcceptance; const identity = api?.fingerprint ? api.fingerprint({ progressId: record?.progress_id || null, state: record?.state || null, reasonCode }) : canonicalize({ progressId: record?.progress_id || null, reasonCode }); const timestamp = record?.state?.updatedAt || record?.updated_at || "1970-01-01T00:00:00.000Z";
      const quarantine = { quarantine_id: `pattern-evolution-acceptance-quarantine:${identity}`, source_store: "progress", source_key: record?.progress_id || null, reason_code: reasonCode, record: clone(record), created_at: timestamp, expires_at: timestamp }; const database = await this._database(); const transaction = database.transaction("quarantine", "readwrite"); transaction.objectStore("quarantine").put(quarantine); await transactionComplete(transaction); return clone(quarantine);
    }

    async listPatternEvolutionAcceptances(projectId, calculationId = null, filters = {}) {
      if (calculationId && typeof calculationId === "object") { filters = calculationId; calculationId = filters.calculationId || null; }
      const project = await this._validatedCurrentProject(projectId); const effectiveCalculationId = calculationId || project.active_calculation_id; if (!effectiveCalculationId) return [];
      const database = await this._database(); const transaction = database.transaction("progress", "readonly"); const records = await requestResult(transaction.objectStore("progress").index("by_calculation_kind").getAll(global.IDBKeyRange.only([effectiveCalculationId, "PATTERN_EVOLUTION_ACCEPTANCE"]))); await transactionComplete(transaction);
      const filtered = records.filter((entry) => entry.project_id === projectId && (!filters.patternId || entry.state?.patternId === filters.patternId) && (!filters.verificationId && !filters.sourceVerificationId || entry.state?.sourceVerificationId === (filters.verificationId || filters.sourceVerificationId)) && (!filters.executionId || entry.state?.sourceExecutionId === filters.executionId) && (!filters.lifecycle || entry.state?.lifecycle === filters.lifecycle) && (!filters.status || entry.state?.status === filters.status) && (!filters.verdict || entry.state?.verdict === filters.verdict) && (filters.importedUnproven === undefined || entry.state?.importedUnproven === filters.importedUnproven));
      filtered.sort((a, b) => a.epoch - b.epoch || a.state.revision - b.state.revision || (String(a.state.id) < String(b.state.id) ? -1 : String(a.state.id) > String(b.state.id) ? 1 : 0)); const valid = [];
      for (const entry of filtered) { try { validateProgressRecord(entry, projectId, effectiveCalculationId, "PATTERN_EVOLUTION_ACCEPTANCE"); validateImportedPatternEvolutionAcceptance(entry.state, projectId); valid.push(clone(entry)); } catch (error) { await this._quarantinePatternEvolutionAcceptance(entry, error?.code || "INVALID_PATTERN_EVOLUTION_ACCEPTANCE"); } } return valid;
    }

    async getPatternEvolutionAcceptance(projectId, acceptanceId = null, calculationId = null, verificationId = null) { const records = await this.listPatternEvolutionAcceptances(projectId, calculationId, verificationId ? { verificationId } : {}); if (acceptanceId) return records.find((entry) => entry.progress_id === acceptanceId || entry.state?.id === acceptanceId || entry.state?.acceptanceId === acceptanceId) || null; return records.at(-1) || null; }
    async getLatestPatternEvolutionAcceptance(projectId, calculationId = null, filters = {}) { const records = await this.listPatternEvolutionAcceptances(projectId, calculationId, filters); const superseded = new Set(records.flatMap((entry) => [entry.state?.predecessorAcceptanceId, entry.state?.supersedesAcceptanceId]).filter(Boolean)); return records.filter((entry) => !superseded.has(entry.state?.id) && entry.state?.lifecycle !== "stale" && !entry.state?.importedUnproven && !entry.state?.collision && !entry.state?.quarantined).at(-1) || null; }
    async getLatestPatternEvolutionAcceptanceForVerification(projectId, verificationId, calculationId = null) { return this.getLatestPatternEvolutionAcceptance(projectId, calculationId, { verificationId }); }
    async getLatestPatternEvolutionAcceptanceForExecution(projectId, executionId, calculationId = null) { return this.getLatestPatternEvolutionAcceptance(projectId, calculationId, { executionId }); }
    async getLatestPatternEvolutionAcceptanceForPattern(projectId, patternId, calculationId = null) { return this.getLatestPatternEvolutionAcceptance(projectId, calculationId, { patternId }); }

    async getPatternEvolutionAcceptanceSourceChain(projectId, acceptanceOrVerificationId, calculationId = null) {
      const acceptance = await this.getPatternEvolutionAcceptance(projectId, acceptanceOrVerificationId, calculationId); const verificationId = acceptance?.state?.sourceVerificationId || acceptanceOrVerificationId; const chain = await this.getPatternEvolutionExecutionVerificationSourceChain(projectId, verificationId, calculationId); return { acceptance, verification: chain.verification, execution: chain.execution, decision: chain.decision, review: chain.review, proposal: chain.proposal, initiation: chain.initiation };
    }

    async _assertPatternEvolutionAcceptanceReferences(projectId, calculationId, state) { const api = global.YarnAIPatternEvolutionAcceptance; if (!api?.loadSource || !api?.calculateSourceGate) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_API_MISSING", "Pattern evolution acceptance module is not loaded."); const source = await api.loadSource(this, projectId, state.sourceVerificationId); if (source.calculationId !== calculationId) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_SCOPE_INVALID", "Acceptance does not belong to the active calculation."); const gate = api.calculateSourceGate(source, state); if (!gate.valid) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_REFERENCE_INVALID", "Acceptance does not resolve to one immutable terminal local verification chain.", { details: { reasons: gate.reasons } }); return { source, gate }; }

    async savePatternEvolutionAcceptance(projectId, calculationOrState, stateOrOptions = {}, maybeOptions = {}) {
      const directState = calculationOrState && typeof calculationOrState === "object"; const state = directState ? calculationOrState : stateOrOptions; const calculationId = directState ? state.calculationId : calculationOrState; const options = directState ? stateOrOptions : maybeOptions; const api = global.YarnAIPatternEvolutionAcceptance; const report = api?.validatePatternEvolutionAcceptance?.(state);
      if (!api || state?.kind !== "PATTERN_EVOLUTION_ACCEPTANCE" || state?.type !== "PATTERN_EVOLUTION_ACCEPTANCE" || state?.projectId !== projectId || state?.calculationId !== calculationId || !report?.valid) throw new ProjectRepositoryError("INVALID_PATTERN_EVOLUTION_ACCEPTANCE_STATE", "Pattern evolution acceptance snapshot is corrupted."); validateImportedPatternEvolutionAcceptance(state, projectId);
      return this._serialize(projectId, async () => {
        const before = await this._validatedCurrentProject(projectId); if (before.active_calculation_id !== calculationId) throw new ProjectRepositoryError("CALCULATION_MISMATCH", "Acceptance does not belong to the active calculation."); const existing = await this.listPatternEvolutionAcceptances(projectId, calculationId); const current = options.recordId ? existing.find((entry) => entry.progress_id === options.recordId || entry.state?.id === options.recordId) : existing.find((entry) => entry.state?.id === state.id); const revalidation = options.allowRevalidation === true && state.predecessorAcceptanceId && existing.some((entry) => entry.state?.id === state.predecessorAcceptanceId);
        if (!current) { const duplicate = api.classifyDuplicate(existing, state); if (["collision", "identity_collision"].includes(duplicate.status)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_COLLISION", "Acceptance identity collision detected."); if (duplicate.status === "duplicate_terminal") throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_DUPLICATE_TERMINAL", "A terminal acceptance already exists for this semantic envelope."); if (duplicate.status === "semantic_duplicate" && !revalidation) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_SEMANTIC_DUPLICATE", "An acceptance already exists for this semantic envelope."); }
        if (!(options.allowImported === true && state.importedUnproven)) await this._assertPatternEvolutionAcceptanceReferences(projectId, calculationId, state); const timestamp = options.timestamp || state.updatedAt; let progress; let operationKind;
        if (current) {
          if ((options.expectedRevision !== undefined && options.expectedRevision !== current.state.revision) || (options.expectedIdentity !== undefined && options.expectedIdentity !== current.state.identity)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_REVISION_CONFLICT", "Acceptance was changed by another operation."); if (canonicalize(state) === canonicalize(current.state)) return clone(current); if (api.TERMINAL_LIFECYCLES.includes(current.state.lifecycle) || current.state.lifecycle === "stale") throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_TERMINAL", "Terminal and stale acceptances are immutable."); if (state.id !== current.state.id || state.acceptanceId !== current.state.acceptanceId || state.epoch !== current.state.epoch || state.revision <= current.state.revision) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_REVISION_CONFLICT", "Acceptance revision or identity is invalid."); if (!api.TRANSITIONS[current.state.lifecycle]?.includes(state.lifecycle) && state.lifecycle !== current.state.lifecycle) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_LIFECYCLE_CONFLICT", "Acceptance lifecycle transition is invalid."); const immutableFields = ["projectId", "patternId", "calculationId", "sourceInitiationId", "sourceProposalId", "sourceReviewId", "sourceDecisionId", "sourceExecutionId", "sourceVerificationId", "sourceVerificationRevision", "sourceVerificationDigest", "sourceChainDigest", "sourceSnapshots", "sourceBinding", "assessmentInput", "acceptancePolicyVersion", "evidencePolicyVersion", "riskPolicyVersion", "identity", "semanticIdentity", "predecessorAcceptanceId", "supersedesAcceptanceId", "epoch", "createdAt"]; if (immutableFields.some((field) => canonicalize(state[field]) !== canonicalize(current.state[field]))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_IMMUTABLE", "Acceptance semantics and source snapshots are immutable."); progress = clone(current); progress.state = clone(state); progress.revision += 1; progress.updated_at = timestamp; operationKind = options.operationKind || `PATTERN_EVOLUTION_ACCEPTANCE_${state.lifecycle.toUpperCase()}`;
        } else { progress = this._initialProgress(projectId, calculationId, "PATTERN_EVOLUTION_ACCEPTANCE", timestamp); progress.epoch = existing.reduce((maximum, entry) => Math.max(maximum, entry.epoch), 0) + 1; progress.state = clone(state); if (progress.state.epoch !== progress.epoch) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_EPOCH_CONFLICT", "Acceptance epoch differs from progress epoch."); operationKind = options.operationKind || (revalidation ? "PATTERN_EVOLUTION_ACCEPTANCE_REVALIDATED" : state.importedUnproven ? "PATTERN_EVOLUTION_ACCEPTANCE_IMPORTED" : "PATTERN_EVOLUTION_ACCEPTANCE_CREATED"); }
        const nextProject = clone(before); nextProject.current_stage = `pattern_evolution_acceptance_${state.lifecycle}`; nextProject.updated_at = timestamp; nextProject.revision += 1; nextProject.materialized_checksum = await checksumPayload(projectChecksumPayload(nextProject)); const operation = createOperation(nextProject, operationKind, { calculation_id: calculationId, progress_id: progress.progress_id, progress_epoch: progress.epoch, progress_revision: progress.revision, progress_state: clone(state) }, timestamp, before.revision, nextProject.revision); const checkpoint = createCheckpoint(nextProject, nextProject.materialized_checksum, nextProject.revision, timestamp);
        const database = await this._database(); const transaction = database.transaction(["projects", "progress", "operations", "checkpoints", "meta"], "readwrite"); try { const storedProject = await requestResult(transaction.objectStore("projects").get(projectId)); const storedProgress = current ? await requestResult(transaction.objectStore("progress").get(current.progress_id)) : null; if (!storedProject || storedProject.revision !== before.revision || current && (!storedProgress || storedProgress.revision !== current.revision)) { transaction.abort(); throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_REVISION_CONFLICT", "Acceptance changed concurrently."); } await allocateOperationMetadata(transaction, operation); current ? transaction.objectStore("progress").put(progress) : transaction.objectStore("progress").add(progress); transaction.objectStore("projects").put(nextProject); transaction.objectStore("operations").add(operation); transaction.objectStore("checkpoints").add(checkpoint); await transactionComplete(transaction); } catch (error) { throw mapStorageError(error); } this._notify(projectId, nextProject.revision, operation.kind); return clone(progress);
      });
    }

    async transitionPatternEvolutionAcceptance(projectId, acceptanceId, nextLifecycle, options = {}) { const current = await this.getPatternEvolutionAcceptance(projectId, acceptanceId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_NOT_FOUND", "Pattern evolution acceptance was not found."); const api = global.YarnAIPatternEvolutionAcceptance; const source = await api.loadSource(this, projectId, current.state.sourceVerificationId); const command = { now: options.timestamp || current.state.updatedAt, expectedRevision: options.expectedRevision ?? current.state.revision, expectedIdentity: options.expectedIdentity ?? current.state.identity }; const commands = { assessing: api.startAssessing, deliberating: api.startDeliberating, reviewing: api.startReviewing, stale: api.markAcceptanceStale }; let next; if (api.TERMINAL_LIFECYCLES.includes(nextLifecycle)) { next = api.finalizeAcceptance(current.state, source, command); if (next.lifecycle !== nextLifecycle) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_LIFECYCLE_CONFLICT", "Requested terminal lifecycle differs from the computed verdict."); } else { const transition = commands[nextLifecycle]; if (!transition) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_LIFECYCLE_CONFLICT", "Acceptance lifecycle transition is unsupported."); next = transition(current.state, source, command); } return this.savePatternEvolutionAcceptance(projectId, next, { recordId: current.progress_id, expectedRevision: current.state.revision, expectedIdentity: current.state.identity, timestamp: next.updatedAt }); }
    async createPatternEvolutionAcceptance(projectId, input = {}) { const api = global.YarnAIPatternEvolutionAcceptance; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_API_MISSING", "Pattern evolution acceptance module is not loaded."); return api.createForProject(this, projectId, input); }
    async readPatternEvolutionAcceptance(projectId, acceptanceId = null, verificationId = null) { const api = global.YarnAIPatternEvolutionAcceptance; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_API_MISSING", "Pattern evolution acceptance module is not loaded."); return api.readForProject(this, projectId, acceptanceId, verificationId); }
    async revalidatePatternEvolutionAcceptance(projectId, acceptanceId, options = {}) { const current = await this.getPatternEvolutionAcceptance(projectId, acceptanceId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_NOT_FOUND", "Pattern evolution acceptance was not found."); const api = global.YarnAIPatternEvolutionAcceptance; const source = await api.loadSource(this, projectId, current.state.sourceVerificationId); const next = api.revalidatePatternEvolutionAcceptance(current.state, source, { ...options, now: options.timestamp || current.state.updatedAt }); return this.savePatternEvolutionAcceptance(projectId, next, { ...options, timestamp: next.updatedAt, operationKind: "PATTERN_EVOLUTION_ACCEPTANCE_REVALIDATED", allowRevalidation: true }); }
    async exportPatternEvolutionAcceptances(projectId, calculationId = null) { const api = global.YarnAIPatternEvolutionAcceptance; const records = await this.listPatternEvolutionAcceptances(projectId, calculationId); return records.map((entry) => api.serializePatternEvolutionAcceptance(entry.state)); }
    async importPatternEvolutionAcceptance(projectId, serialized, options = {}) { const api = global.YarnAIPatternEvolutionAcceptance; const existing = await this.listPatternEvolutionAcceptances(projectId, options.calculationId || null); const result = api.importPatternEvolutionAcceptance(existing, serialized, options); if (["malformed", "collision"].includes(result.status)) { await this._quarantinePatternEvolutionAcceptance({ progress_id: null, state: serialized, updated_at: options.timestamp }, result.quarantine?.reasonCode || "INVALID_PATTERN_EVOLUTION_ACCEPTANCE_IMPORT"); return result; } if (!result.changed) return result; const stored = await this.savePatternEvolutionAcceptance(projectId, result.record, { timestamp: options.timestamp || result.record.updatedAt, allowImported: true }); return { ...result, stored }; }

    async _quarantinePatternEvolutionClosure(record, reasonCode) {
      const api = global.YarnAIPatternEvolutionClosure; const identity = api?.fingerprint ? api.fingerprint({ progressId: record?.progress_id || null, state: record?.state || null, reasonCode }) : canonicalize({ progressId: record?.progress_id || null, reasonCode }); const timestamp = record?.state?.updatedAt || record?.updated_at || "1970-01-01T00:00:00.000Z";
      const quarantine = { quarantine_id: `pattern-evolution-closure-quarantine:${identity}`, source_store: "progress", source_key: record?.progress_id || null, reason_code: reasonCode, record: clone(record), created_at: timestamp, expires_at: timestamp }; const database = await this._database(); const transaction = database.transaction("quarantine", "readwrite"); transaction.objectStore("quarantine").put(quarantine); await transactionComplete(transaction); return clone(quarantine);
    }

    async listPatternEvolutionClosures(projectId, calculationId = null, filters = {}) {
      if (calculationId && typeof calculationId === "object") { filters = calculationId; calculationId = filters.calculationId || null; }
      const project = await this._validatedCurrentProject(projectId); const effectiveCalculationId = calculationId || project.active_calculation_id; if (!effectiveCalculationId) return [];
      const database = await this._database(); const transaction = database.transaction("progress", "readonly"); const records = await requestResult(transaction.objectStore("progress").index("by_calculation_kind").getAll(global.IDBKeyRange.only([effectiveCalculationId, "PATTERN_EVOLUTION_CLOSURE"]))); await transactionComplete(transaction);
      const filtered = records.filter((entry) => entry.project_id === projectId && (!filters.patternId || entry.state?.patternId === filters.patternId) && (!filters.evolutionId || entry.state?.evolutionId === filters.evolutionId) && (!filters.cycleId || entry.state?.cycleId === filters.cycleId) && (!filters.acceptanceId && !filters.sourceAcceptanceId || entry.state?.sourceAcceptanceId === (filters.acceptanceId || filters.sourceAcceptanceId)) && (!filters.verificationId || entry.state?.sourceVerificationId === filters.verificationId) && (!filters.lifecycle || entry.state?.lifecycle === filters.lifecycle) && (!filters.status || entry.state?.status === filters.status) && (!filters.outcome || entry.state?.outcome === filters.outcome) && (filters.importedUnproven === undefined || entry.state?.importedUnproven === filters.importedUnproven));
      filtered.sort((a, b) => a.epoch - b.epoch || a.state.closureRevision - b.state.closureRevision || a.state.revision - b.state.revision || (String(a.state.id) < String(b.state.id) ? -1 : String(a.state.id) > String(b.state.id) ? 1 : 0)); const valid = [];
      for (const entry of filtered) { try { validateProgressRecord(entry, projectId, effectiveCalculationId, "PATTERN_EVOLUTION_CLOSURE"); validateImportedPatternEvolutionClosure(entry.state, projectId); valid.push(clone(entry)); } catch (error) { await this._quarantinePatternEvolutionClosure(entry, error?.code || "INVALID_PATTERN_EVOLUTION_CLOSURE"); } } return valid;
    }

    async getPatternEvolutionClosure(projectId, closureId = null, calculationId = null, acceptanceId = null) { const records = await this.listPatternEvolutionClosures(projectId, calculationId, acceptanceId ? { acceptanceId } : {}); if (closureId) return records.find((entry) => entry.progress_id === closureId || entry.state?.id === closureId || entry.state?.closureId === closureId) || null; return records.at(-1) || null; }
    async getLatestPatternEvolutionClosure(projectId, calculationId = null, filters = {}) { const records = await this.listPatternEvolutionClosures(projectId, calculationId, filters); const superseded = new Set(records.flatMap((entry) => [entry.state?.predecessorClosureId, entry.state?.supersedesClosureId]).filter(Boolean)); return records.filter((entry) => !superseded.has(entry.state?.id) && !entry.state?.importedUnproven && !entry.state?.collision && !entry.state?.quarantined).at(-1) || null; }
    async getLatestPatternEvolutionClosureForAcceptance(projectId, acceptanceId, calculationId = null) { return this.getLatestPatternEvolutionClosure(projectId, calculationId, { acceptanceId }); }
    async getLatestPatternEvolutionClosureForPattern(projectId, patternId, calculationId = null) { return this.getLatestPatternEvolutionClosure(projectId, calculationId, { patternId }); }
    async getLatestPatternEvolutionClosureForEvolution(projectId, evolutionId, calculationId = null) { return this.getLatestPatternEvolutionClosure(projectId, calculationId, { evolutionId }); }
    async lookupPatternEvolutionClosureBySourceAcceptance(projectId, acceptanceId, calculationId = null) { return this.getLatestPatternEvolutionClosureForAcceptance(projectId, acceptanceId, calculationId); }

    async getPatternEvolutionClosureSourceChain(projectId, closureOrAcceptanceId, calculationId = null) { const closure = await this.getPatternEvolutionClosure(projectId, closureOrAcceptanceId, calculationId); const acceptanceId = closure?.state?.sourceAcceptanceId || closureOrAcceptanceId; const chain = await this.getPatternEvolutionAcceptanceSourceChain(projectId, acceptanceId, calculationId); return { closure, acceptance: chain.acceptance, verification: chain.verification, execution: chain.execution, decision: chain.decision, review: chain.review, proposal: chain.proposal, initiation: chain.initiation }; }
    async resolvePatternEvolutionClosureLiveChain(projectId, closureOrAcceptanceId, calculationId = null) { return this.getPatternEvolutionClosureSourceChain(projectId, closureOrAcceptanceId, calculationId); }
    async detectPatternEvolutionClosureDuplicates(projectId, candidate, calculationId = null) { const api = global.YarnAIPatternEvolutionClosure; const records = await this.listPatternEvolutionClosures(projectId, calculationId || candidate?.calculationId); return api.classifyDuplicate(records, candidate); }
    async detectPatternEvolutionClosureCollision(projectId, candidate, calculationId = null) { const result = await this.detectPatternEvolutionClosureDuplicates(projectId, candidate, calculationId); return ["collision", "identity_collision"].includes(result.status) ? result : null; }
    async detectPatternEvolutionClosureSuperseding(projectId, closureId, calculationId = null) { const records = await this.listPatternEvolutionClosures(projectId, calculationId); return records.find((entry) => entry.state?.predecessorClosureId === closureId || entry.state?.supersedesClosureId === closureId) || null; }

    async _assertPatternEvolutionClosureReferences(projectId, calculationId, state) { const api = global.YarnAIPatternEvolutionClosure; if (!api?.loadSource || !api?.calculateLiveChainGate) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_API_MISSING", "Pattern evolution closure module is not loaded."); const source = await api.loadSource(this, projectId, state.sourceAcceptanceId); if (!source.acceptance || source.calculationId !== calculationId) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_SCOPE_INVALID", "Closure does not belong to one live acceptance cycle."); const gate = api.calculateLiveChainGate(source, state); if (state.sourceAcceptanceId !== source.acceptance.id || state.sourceAcceptanceRevision !== source.acceptance.revision || state.sourceAcceptanceDigest !== (source.acceptance.digest || source.acceptance.identity)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_REFERENCE_INVALID", "Closure acceptance binding is not current.", { details: { reasons: gate.reasons } }); return { source, gate }; }

    async savePatternEvolutionClosure(projectId, calculationOrState, stateOrOptions = {}, maybeOptions = {}) {
      const directState = calculationOrState && typeof calculationOrState === "object"; const state = directState ? calculationOrState : stateOrOptions; const calculationId = directState ? state.calculationId : calculationOrState; const options = directState ? stateOrOptions : maybeOptions; const api = global.YarnAIPatternEvolutionClosure; const report = api?.validatePatternEvolutionClosure?.(state);
      if (!api || state?.kind !== "PATTERN_EVOLUTION_CLOSURE" || state?.type !== "PATTERN_EVOLUTION_CLOSURE" || state?.projectId !== projectId || state?.calculationId !== calculationId || !report?.valid) throw new ProjectRepositoryError("INVALID_PATTERN_EVOLUTION_CLOSURE_STATE", "Pattern evolution closure snapshot is corrupted."); validateImportedPatternEvolutionClosure(state, projectId);
      return this._serialize(projectId, async () => {
        const before = await this._validatedCurrentProject(projectId); if (before.active_calculation_id !== calculationId) throw new ProjectRepositoryError("CALCULATION_MISMATCH", "Closure does not belong to the active calculation."); const existing = await this.listPatternEvolutionClosures(projectId, calculationId); const current = options.recordId ? existing.find((entry) => entry.progress_id === options.recordId || entry.state?.id === options.recordId) : existing.find((entry) => entry.state?.id === state.id); const revalidation = options.allowRevalidation === true && state.predecessorClosureId && existing.some((entry) => entry.state?.id === state.predecessorClosureId);
        if (!current) { const duplicate = api.classifyDuplicate(existing, state); if (["collision", "identity_collision"].includes(duplicate.status)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_COLLISION", "Closure identity collision detected."); if (duplicate.status === "duplicate_terminal") throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_DUPLICATE_TERMINAL", "A competing terminal closure already exists."); if (duplicate.status === "semantic_duplicate" && !revalidation) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_SEMANTIC_DUPLICATE", "A closure already exists for this semantic cycle."); }
        if (!(options.allowImported === true && state.importedUnproven)) await this._assertPatternEvolutionClosureReferences(projectId, calculationId, state); const timestamp = options.timestamp || state.updatedAt; let progress; let operationKind;
        if (current) {
          if ((options.expectedRevision !== undefined && options.expectedRevision !== current.state.revision) || (options.expectedIdentity !== undefined && options.expectedIdentity !== current.state.identity)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_REVISION_CONFLICT", "Closure changed concurrently."); if (canonicalize(state) === canonicalize(current.state)) return clone(current); if (api.TERMINAL_LIFECYCLES.includes(current.state.lifecycle)) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_TERMINAL", "Terminal closures are immutable."); if (state.id !== current.state.id || state.closureId !== current.state.closureId || state.epoch !== current.state.epoch || state.revision <= current.state.revision) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_REVISION_CONFLICT", "Closure revision or identity is invalid."); if (!api.TRANSITIONS[current.state.lifecycle]?.includes(state.lifecycle) && state.lifecycle !== current.state.lifecycle) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_LIFECYCLE_CONFLICT", "Closure lifecycle transition is invalid."); const immutableFields = ["projectId", "patternId", "calculationId", "evolutionId", "cycleId", "sourceInitiationId", "sourceProposalId", "sourceReviewId", "sourceDecisionId", "sourceExecutionId", "sourceVerificationId", "sourceAcceptanceId", "sourceAcceptanceRevision", "sourceAcceptanceDigest", "sourceChainDigest", "sourceSnapshots", "sourceBinding", "reconciliationInput", "closurePolicyVersion", "evidencePolicyVersion", "riskPolicyVersion", "identity", "semanticIdentity", "predecessorClosureId", "supersedesClosureId", "closureRevision", "epoch", "createdAt"]; if (immutableFields.some((field) => canonicalize(state[field]) !== canonicalize(current.state[field]))) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_IMMUTABLE", "Closure semantics and source snapshots are immutable."); progress = clone(current); progress.state = clone(state); progress.revision += 1; progress.updated_at = timestamp; operationKind = options.operationKind || `PATTERN_EVOLUTION_CLOSURE_${state.lifecycle.toUpperCase()}`;
        } else { progress = this._initialProgress(projectId, calculationId, "PATTERN_EVOLUTION_CLOSURE", timestamp); progress.epoch = existing.reduce((maximum, entry) => Math.max(maximum, entry.epoch), 0) + 1; progress.state = clone(state); if (progress.state.epoch !== progress.epoch) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_EPOCH_CONFLICT", "Closure epoch differs from progress epoch."); operationKind = options.operationKind || (revalidation ? "PATTERN_EVOLUTION_CLOSURE_REVALIDATED" : state.importedUnproven ? "PATTERN_EVOLUTION_CLOSURE_IMPORTED" : "PATTERN_EVOLUTION_CLOSURE_CREATED"); }
        const nextProject = clone(before); nextProject.current_stage = `pattern_evolution_closure_${state.lifecycle}`; nextProject.updated_at = timestamp; nextProject.revision += 1; nextProject.materialized_checksum = await checksumPayload(projectChecksumPayload(nextProject)); const operation = createOperation(nextProject, operationKind, { calculation_id: calculationId, progress_id: progress.progress_id, progress_epoch: progress.epoch, progress_revision: progress.revision, progress_state: clone(state) }, timestamp, before.revision, nextProject.revision); const checkpoint = createCheckpoint(nextProject, nextProject.materialized_checksum, nextProject.revision, timestamp);
        const database = await this._database(); const transaction = database.transaction(["projects", "progress", "operations", "checkpoints", "meta"], "readwrite"); try { const storedProject = await requestResult(transaction.objectStore("projects").get(projectId)); const storedProgress = current ? await requestResult(transaction.objectStore("progress").get(current.progress_id)) : null; if (!storedProject || storedProject.revision !== before.revision || current && (!storedProgress || storedProgress.revision !== current.revision)) { transaction.abort(); throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_REVISION_CONFLICT", "Closure changed concurrently."); } await allocateOperationMetadata(transaction, operation); current ? transaction.objectStore("progress").put(progress) : transaction.objectStore("progress").add(progress); transaction.objectStore("projects").put(nextProject); transaction.objectStore("operations").add(operation); transaction.objectStore("checkpoints").add(checkpoint); await transactionComplete(transaction); } catch (error) { throw mapStorageError(error); } this._notify(projectId, nextProject.revision, operation.kind); return clone(progress);
      });
    }

    async transitionPatternEvolutionClosure(projectId, closureId, nextLifecycle, options = {}) { const current = await this.getPatternEvolutionClosure(projectId, closureId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_NOT_FOUND", "Pattern evolution closure was not found."); const api = global.YarnAIPatternEvolutionClosure; const source = await api.loadSource(this, projectId, current.state.sourceAcceptanceId); const command = { now: options.timestamp || current.state.updatedAt, expectedRevision: options.expectedRevision ?? current.state.revision, expectedIdentity: options.expectedIdentity ?? current.state.identity }; const commands = { reconciling: api.startReconciling, finalizing: api.startFinalizing, reviewing: api.startReviewing }; let next; if (api.TERMINAL_LIFECYCLES.includes(nextLifecycle)) { next = api.finalizeClosure(current.state, source, command); if (next.lifecycle !== nextLifecycle) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_LIFECYCLE_CONFLICT", "Requested terminal lifecycle differs from the computed outcome."); } else { const transition = commands[nextLifecycle]; if (!transition) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_LIFECYCLE_CONFLICT", "Closure lifecycle transition is unsupported."); next = transition(current.state, source, command); } return this.savePatternEvolutionClosure(projectId, next, { recordId: current.progress_id, expectedRevision: current.state.revision, expectedIdentity: current.state.identity, timestamp: next.updatedAt }); }
    async createPatternEvolutionClosure(projectId, input = {}) { const api = global.YarnAIPatternEvolutionClosure; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_API_MISSING", "Pattern evolution closure module is not loaded."); return api.createForProject(this, projectId, input); }
    async readPatternEvolutionClosure(projectId, closureId = null, acceptanceId = null) { const api = global.YarnAIPatternEvolutionClosure; if (!api) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_API_MISSING", "Pattern evolution closure module is not loaded."); return api.readForProject(this, projectId, closureId, acceptanceId); }
    async revalidatePatternEvolutionClosure(projectId, closureId, options = {}) { const current = await this.getPatternEvolutionClosure(projectId, closureId); if (!current) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_NOT_FOUND", "Pattern evolution closure was not found."); const api = global.YarnAIPatternEvolutionClosure; const source = await api.loadSource(this, projectId, current.state.sourceAcceptanceId); const next = api.revalidatePatternEvolutionClosure(current.state, source, { ...options, now: options.timestamp || current.state.updatedAt }); return this.savePatternEvolutionClosure(projectId, next, { ...options, timestamp: next.updatedAt, operationKind: "PATTERN_EVOLUTION_CLOSURE_REVALIDATED", allowRevalidation: true }); }
    async projectPatternEvolutionClosureStale(projectId, closureId, options = {}) { const current = await this.getPatternEvolutionClosure(projectId, closureId, options.calculationId || null); if (!current) return null; const api = global.YarnAIPatternEvolutionClosure; const source = await api.loadSource(this, projectId, current.state.sourceAcceptanceId); return api.projectPatternEvolutionClosure(current.state, source); }
    async exportPatternEvolutionClosures(projectId, calculationId = null) { const api = global.YarnAIPatternEvolutionClosure; const records = await this.listPatternEvolutionClosures(projectId, calculationId); return records.map((entry) => api.serializePatternEvolutionClosure(entry.state)); }
    async importPatternEvolutionClosure(projectId, serialized, options = {}) { const api = global.YarnAIPatternEvolutionClosure; const existing = await this.listPatternEvolutionClosures(projectId, options.calculationId || null); const result = api.importPatternEvolutionClosure(existing, serialized, options); if (["malformed", "collision"].includes(result.status)) { await this._quarantinePatternEvolutionClosure({ progress_id: null, state: serialized, updated_at: options.timestamp }, result.quarantine?.reasonCode || "INVALID_PATTERN_EVOLUTION_CLOSURE_IMPORT"); return result; } if (!result.changed) return result; const stored = await this.savePatternEvolutionClosure(projectId, result.record, { timestamp: options.timestamp || result.record.updatedAt, allowImported: true }); return { ...result, stored }; }
    ...
    async importProject(source) {
      ...
      const importedProgress = progress.map((entry) => {
        ...
        if (entry.kind === "PATTERN_EVOLUTION_INITIATION") {
          validateImportedPatternEvolutionInitiation(importedState, sourceProjectId);
        }
        if (entry.kind === "PATTERN_EVOLUTION_PROPOSAL") {
          validateImportedPatternEvolutionProposal(importedState, sourceProjectId);
        }
        if (entry.kind === "PATTERN_EVOLUTION_PROPOSAL_REVIEW") {
          validateImportedPatternEvolutionProposalReview(importedState, sourceProjectId);
        }
        if (entry.kind === "PATTERN_EVOLUTION_DECISION") {
          validateImportedPatternEvolutionDecision(importedState, sourceProjectId);
        }
        if (entry.kind === "PATTERN_EVOLUTION_EXECUTION") {
          validateImportedPatternEvolutionExecution(importedState, sourceProjectId);
        }
        if (entry.kind === "PATTERN_EVOLUTION_EXECUTION_VERIFICATION") {
          validateImportedPatternEvolutionExecutionVerification(importedState, sourceProjectId);
        }
        if (entry.kind === "PATTERN_EVOLUTION_ACCEPTANCE") {
          validateImportedPatternEvolutionAcceptance(importedState, sourceProjectId);
        }
        if (entry.kind === "PATTERN_EVOLUTION_CLOSURE") {
          validateImportedPatternEvolutionClosure(importedState, sourceProjectId);
        }
        if (
          collision &&
          ...
          entry.kind !== "PATTERN_EVOLUTION_INITIATION" &&
          entry.kind !== "PATTERN_EVOLUTION_PROPOSAL" &&
          entry.kind !== "PATTERN_EVOLUTION_PROPOSAL_REVIEW" &&
          entry.kind !== "PATTERN_EVOLUTION_DECISION" &&
          entry.kind !== "PATTERN_EVOLUTION_EXECUTION" &&
          entry.kind !== "PATTERN_EVOLUTION_EXECUTION_VERIFICATION" &&
          entry.kind !== "PATTERN_EVOLUTION_ACCEPTANCE" &&
          entry.kind !== "PATTERN_EVOLUTION_CLOSURE" &&
      ...
      importedProgress.forEach((entry, entryIndex) => {
        if (entry.kind !== "PATTERN_EVOLUTION_INITIATION") return;
        const initiationApi = global.YarnAIPatternEvolutionInitiation;
        if (!initiationApi?.remapPatternEvolutionInitiation || !initiationApi?.makeImportedPatternEvolutionInitiationUnproven || !initiationApi?.sourceSnapshot) throw new ProjectRepositoryError("PATTERN_EVOLUTION_INITIATION_API_MISSING", "Pattern evolution initiation module is not loaded.");
        const originalState = progress[entryIndex]?.state; const originalClosure = progress.find((candidate) => candidate.kind === "PATTERN_EXECUTION_ADAPTATION_CLOSURE" && candidate.state?.id === originalState?.sourceClosureId);
        if (!originalClosure || canonicalize(originalState.sourceSnapshots?.closure) !== canonicalize(initiationApi.sourceSnapshot(originalClosure.state, "closure"))) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_INITIATION_CLOSURE_SNAPSHOT", "Evolution initiation closure snapshot does not match the imported closure.");
        let state = entry.state;
        if (collision) { const referenceMap = new Map([[sourceProjectId, projectId], ...calculationMap.entries(), ...progressMap.entries()]); for (let index = 0; index < progress.length; index += 1) { const beforeState = progress[index]?.state; const afterState = importedProgress[index]?.state; for (const field of ["id", "retrospectiveId", "learningId", "adaptationId", "adaptationValidationId", "validationId", "adaptationPromotionId", "promotionId", "adaptationRolloutId", "rolloutId", "evaluationId", "closureId", "initiationId", "identity"]) if (beforeState?.[field] && afterState?.[field]) referenceMap.set(beforeState[field], afterState[field]); } state = clone(initiationApi.remapPatternEvolutionInitiation(state, referenceMap)); }
        const linkedClosure = importedProgress.find((candidate) => candidate.kind === "PATTERN_EXECUTION_ADAPTATION_CLOSURE" && candidate.state?.id === state.sourceClosureId); if (!linkedClosure) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_INITIATION_REFERENCE", "Evolution initiation references a missing adaptation closure.");
        if (!state.importedUnproven) state = clone(initiationApi.makeImportedPatternEvolutionInitiationUnproven(state, { now: state.updatedAt, collision, reason: "import_identity_unproven" })); entry.state = state; validateImportedPatternEvolutionInitiation(state, projectId);
      });
      const submittedProposalIdentities = new Set();
      importedProgress.forEach((entry, entryIndex) => {
        if (entry.kind !== "PATTERN_EVOLUTION_PROPOSAL") return;
        const proposalApi = global.YarnAIPatternEvolutionProposal;
        if (!proposalApi?.remapPatternEvolutionProposal || !proposalApi?.makeImportedPatternEvolutionProposalUnproven || !proposalApi?.sourceSnapshot || !proposalApi?.duplicateIdentity) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_API_MISSING", "Pattern evolution proposal module is not loaded.");
        const originalState = progress[entryIndex]?.state; const originalInitiation = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_INITIATION" && candidate.state?.id === originalState?.sourceInitiationId);
        if (!originalInitiation || canonicalize(originalState.sourceSnapshot) !== canonicalize(proposalApi.sourceSnapshot(originalInitiation.state))) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL_INITIATION_SNAPSHOT", "Evolution proposal initiation snapshot does not match the imported initiation.");
        if (originalState.status === "submitted") { const duplicateKey = proposalApi.duplicateIdentity(originalState); if (submittedProposalIdentities.has(duplicateKey)) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL_DUPLICATE_SUBMITTED", "Import contains duplicate submitted evolution proposals."); submittedProposalIdentities.add(duplicateKey); }
        let state = entry.state;
        if (collision) {
          const referenceMap = new Map([[sourceProjectId, projectId], ...calculationMap.entries(), ...progressMap.entries()]);
          for (let index = 0; index < progress.length; index += 1) { const beforeState = progress[index]?.state; const afterState = importedProgress[index]?.state; for (const field of ["id", "closureId", "initiationId", "proposalId", "identity"]) if (beforeState?.[field] && afterState?.[field]) referenceMap.set(beforeState[field], afterState[field]); for (const collection of ["successCriteria", "proposedChanges", "migrationRequirements"]) for (let itemIndex = 0; itemIndex < (beforeState?.[collection] || []).length; itemIndex += 1) { const beforeItem = beforeState[collection][itemIndex]; const afterItem = afterState?.[collection]?.[itemIndex]; if (beforeItem?.id && afterItem?.id) referenceMap.set(beforeItem.id, afterItem.id); } }
          state = clone(proposalApi.remapPatternEvolutionProposal(state, referenceMap));
        }
        const linkedInitiation = importedProgress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_INITIATION" && candidate.state?.id === state.sourceInitiationId); if (!linkedInitiation) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL_REFERENCE", "Evolution proposal references a missing pattern evolution initiation.");
        if (!state.importedUnproven) state = clone(proposalApi.makeImportedPatternEvolutionProposalUnproven(state, { now: state.updatedAt, collision, reason: "import_identity_unproven" })); entry.state = state; validateImportedPatternEvolutionProposal(state, projectId);
      });
      const importedReviewIdentities = new Set();
      importedProgress.forEach((entry, entryIndex) => {
        if (entry.kind !== "PATTERN_EVOLUTION_PROPOSAL_REVIEW") return;
        const reviewApi = global.YarnAIPatternEvolutionProposalReview;
        if (!reviewApi?.remapPatternEvolutionProposalReview || !reviewApi?.makeImportedPatternEvolutionProposalReviewUnproven || !reviewApi?.proposalSnapshot || !reviewApi?.duplicateIdentity) throw new ProjectRepositoryError("PATTERN_EVOLUTION_PROPOSAL_REVIEW_API_MISSING", "Pattern evolution proposal review module is not loaded.");
        const originalState = progress[entryIndex]?.state; const originalProposal = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_PROPOSAL" && candidate.state?.id === originalState?.sourceProposalId);
        if (!originalProposal || canonicalize(originalState.sourceProposalSnapshot) !== canonicalize(reviewApi.proposalSnapshot(originalProposal.state))) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL_REVIEW_SOURCE_SNAPSHOT", "Proposal review snapshot does not match the imported proposal.");
        const reviewIdentity = reviewApi.duplicateIdentity(originalState); if (importedReviewIdentities.has(reviewIdentity)) throw new ProjectRepositoryError(originalState.status === "approved" ? "INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL_REVIEW_DUPLICATE_APPROVED" : "INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL_REVIEW_DUPLICATE_IDENTITY", "Import contains duplicate proposal review identities."); importedReviewIdentities.add(reviewIdentity);
        let state = entry.state;
        if (collision) {
          const referenceMap = new Map([[sourceProjectId, projectId], ...calculationMap.entries(), ...progressMap.entries()]);
          for (let index = 0; index < progress.length; index += 1) { const beforeState = progress[index]?.state; const afterState = importedProgress[index]?.state; for (const field of ["id", "closureId", "initiationId", "proposalId", "reviewId", "identity"]) if (beforeState?.[field] && afterState?.[field]) referenceMap.set(beforeState[field], afterState[field]); for (const collection of ["successCriteria", "proposedChanges", "migrationRequirements", "evidence", "findings"]) for (let itemIndex = 0; itemIndex < (beforeState?.[collection] || []).length; itemIndex += 1) { const beforeItem = beforeState[collection][itemIndex]; const afterItem = afterState?.[collection]?.[itemIndex]; if (beforeItem?.id && afterItem?.id) referenceMap.set(beforeItem.id, afterItem.id); } }
          state = clone(reviewApi.remapPatternEvolutionProposalReview(state, referenceMap));
        }
        const linkedProposal = importedProgress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_PROPOSAL" && candidate.state?.id === state.sourceProposalId); if (!linkedProposal) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_PROPOSAL_REVIEW_REFERENCE", "Proposal review references a missing proposal.");
        if (!state.importedUnproven) state = clone(reviewApi.makeImportedPatternEvolutionProposalReviewUnproven(state, { now: state.updatedAt, collision, reason: "import_identity_unproven" })); entry.state = state; validateImportedPatternEvolutionProposalReview(state, projectId);
      });
      const importedDecisionIdentities = new Set();
      importedProgress.forEach((entry, entryIndex) => {
        if (entry.kind !== "PATTERN_EVOLUTION_DECISION") return;
        const decisionApi = global.YarnAIPatternEvolutionDecision;
        if (!decisionApi?.remapPatternEvolutionDecision || !decisionApi?.makeImportedPatternEvolutionDecisionUnproven || !decisionApi?.calculateDecisionIdentity) throw new ProjectRepositoryError("PATTERN_EVOLUTION_DECISION_API_MISSING", "Pattern evolution decision module is not loaded.");
        const originalState = progress[entryIndex]?.state; const originalReview = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_PROPOSAL_REVIEW" && candidate.state?.id === originalState?.sourceReviewId); const originalProposal = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_PROPOSAL" && candidate.state?.id === originalState?.sourceProposalId); const originalInitiation = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_INITIATION" && candidate.state?.id === originalState?.sourceInitiationId);
        if (!originalReview || !originalProposal || !originalInitiation || canonicalize(originalState.sourceSnapshots?.review) !== canonicalize(originalReview.state) || canonicalize(originalState.sourceSnapshots?.proposal) !== canonicalize(originalProposal.state) || canonicalize(originalState.sourceSnapshots?.initiation) !== canonicalize(originalInitiation.state)) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_DECISION_SOURCE_SNAPSHOT", "Decision source snapshots do not match the imported immutable source chain.");
        const decisionIdentity = decisionApi.calculateDecisionIdentity(originalState); if (importedDecisionIdentities.has(decisionIdentity)) throw new ProjectRepositoryError(originalState.lifecycle === "authorized" ? "INVALID_IMPORT_PATTERN_EVOLUTION_DECISION_DUPLICATE_AUTHORIZED" : "INVALID_IMPORT_PATTERN_EVOLUTION_DECISION_DUPLICATE_IDENTITY", "Import contains duplicate pattern evolution decision identities."); importedDecisionIdentities.add(decisionIdentity);
        let state = entry.state;
        if (collision) {
          const referenceMap = new Map([[sourceProjectId, projectId], ...calculationMap.entries(), ...progressMap.entries()]);
          for (let index = 0; index < progress.length; index += 1) { const beforeState = progress[index]?.state; const afterState = importedProgress[index]?.state; for (const field of ["id", "initiationId", "proposalId", "reviewId", "decisionId", "identity"]) if (beforeState?.[field] && afterState?.[field]) referenceMap.set(beforeState[field], afterState[field]); for (const collection of ["successCriteria", "proposedChanges", "migrationRequirements", "evidence", "findings", "conditions"]) for (let itemIndex = 0; itemIndex < (beforeState?.[collection] || []).length; itemIndex += 1) { const beforeItem = beforeState[collection][itemIndex]; const afterItem = afterState?.[collection]?.[itemIndex]; if (beforeItem?.id && afterItem?.id) referenceMap.set(beforeItem.id, afterItem.id); } }
          state = clone(decisionApi.remapPatternEvolutionDecision(state, referenceMap));
        }
        const linkedReview = importedProgress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_PROPOSAL_REVIEW" && candidate.state?.id === state.sourceReviewId); const linkedProposal = importedProgress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_PROPOSAL" && candidate.state?.id === state.sourceProposalId); const linkedInitiation = importedProgress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_INITIATION" && candidate.state?.id === state.sourceInitiationId); if (!linkedReview || !linkedProposal || !linkedInitiation) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_DECISION_REFERENCE", "Decision references a missing initiation, proposal, or review.");
        if (!state.importedUnproven) state = clone(decisionApi.makeImportedPatternEvolutionDecisionUnproven(state, { now: state.updatedAt, collision, reason: "import_identity_unproven" })); entry.state = state; validateImportedPatternEvolutionDecision(state, projectId);
      });
      const importedExecutionIdentities = new Set();
      importedProgress.forEach((entry, entryIndex) => {
        if (entry.kind !== "PATTERN_EVOLUTION_EXECUTION") return;
        const executionApi = global.YarnAIPatternEvolutionExecution;
        if (!executionApi?.remapPatternEvolutionExecution || !executionApi?.makeImportedPatternEvolutionExecutionUnproven || !executionApi?.calculateExecutionIdentity) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_API_MISSING", "Pattern evolution execution module is not loaded.");
        const originalState = progress[entryIndex]?.state; const originalDecision = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_DECISION" && candidate.state?.id === originalState?.sourceDecisionId); const originalReview = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_PROPOSAL_REVIEW" && candidate.state?.id === originalState?.sourceReviewId); const originalProposal = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_PROPOSAL" && candidate.state?.id === originalState?.sourceProposalId); const originalInitiation = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_INITIATION" && candidate.state?.id === originalState?.sourceInitiationId);
        if (!originalDecision || !originalReview || !originalProposal || !originalInitiation || canonicalize(originalState.sourceSnapshots?.decision) !== canonicalize(originalDecision.state) || canonicalize(originalState.sourceSnapshots?.review) !== canonicalize(originalReview.state) || canonicalize(originalState.sourceSnapshots?.proposal) !== canonicalize(originalProposal.state) || canonicalize(originalState.sourceSnapshots?.initiation) !== canonicalize(originalInitiation.state)) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION_SOURCE_SNAPSHOT", "Execution source snapshots do not match the imported immutable source chain.");
        const executionIdentity = executionApi.calculateExecutionIdentity(originalState); if (importedExecutionIdentities.has(executionIdentity)) throw new ProjectRepositoryError(originalState.lifecycle === "completed" ? "INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION_DUPLICATE_COMPLETED" : "INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION_DUPLICATE_IDENTITY", "Import contains duplicate pattern evolution execution identities."); importedExecutionIdentities.add(executionIdentity);
        let state = entry.state;
        if (collision) {
          const referenceMap = new Map([[sourceProjectId, projectId], ...calculationMap.entries(), ...progressMap.entries()]);
          for (let index = 0; index < progress.length; index += 1) { const beforeState = progress[index]?.state; const afterState = importedProgress[index]?.state; for (const field of ["id", "initiationId", "proposalId", "reviewId", "decisionId", "executionId", "identity"]) if (beforeState?.[field] && afterState?.[field]) referenceMap.set(beforeState[field], afterState[field]); }
          state = clone(executionApi.remapPatternEvolutionExecution(state, referenceMap));
        }
        const linkedDecision = importedProgress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_DECISION" && candidate.state?.id === state.sourceDecisionId); if (!linkedDecision) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION_REFERENCE", "Execution references a missing pattern evolution decision.");
        if (!state.importedUnproven) state = clone(executionApi.makeImportedPatternEvolutionExecutionUnproven(state, { now: state.updatedAt, collision, reason: "import_identity_unproven" })); entry.state = state; validateImportedPatternEvolutionExecution(state, projectId);
      });
      const importedVerificationIdentities = new Set();
      importedProgress.forEach((entry, entryIndex) => {
        if (entry.kind !== "PATTERN_EVOLUTION_EXECUTION_VERIFICATION") return;
        const verificationApi = global.YarnAIPatternEvolutionExecutionVerification;
        if (!verificationApi?.remapPatternEvolutionExecutionVerification || !verificationApi?.makeImportedPatternEvolutionExecutionVerificationUnproven || !verificationApi?.calculateVerificationIdentity) throw new ProjectRepositoryError("PATTERN_EVOLUTION_EXECUTION_VERIFICATION_API_MISSING", "Pattern evolution execution verification module is not loaded.");
        const originalState = progress[entryIndex]?.state; const originalExecution = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_EXECUTION" && candidate.state?.id === originalState?.sourceExecutionId); const originalDecision = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_DECISION" && candidate.state?.id === originalState?.sourceDecisionId); const originalReview = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_PROPOSAL_REVIEW" && candidate.state?.id === originalState?.sourceReviewId); const originalProposal = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_PROPOSAL" && candidate.state?.id === originalState?.sourceProposalId); const originalInitiation = progress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_INITIATION" && candidate.state?.id === originalState?.sourceInitiationId);
        if (!originalExecution || !originalDecision || !originalReview || !originalProposal || !originalInitiation || canonicalize(originalState.sourceSnapshots?.execution) !== canonicalize(originalExecution.state) || canonicalize(originalState.sourceSnapshots?.decision) !== canonicalize(originalDecision.state) || canonicalize(originalState.sourceSnapshots?.review) !== canonicalize(originalReview.state) || canonicalize(originalState.sourceSnapshots?.proposal) !== canonicalize(originalProposal.state) || canonicalize(originalState.sourceSnapshots?.initiation) !== canonicalize(originalInitiation.state)) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION_VERIFICATION_SOURCE_SNAPSHOT", "Verification source snapshots do not match the imported immutable source chain.");
        const verificationIdentity = verificationApi.calculateVerificationIdentity(originalState); if (importedVerificationIdentities.has(verificationIdentity)) throw new ProjectRepositoryError(originalState.lifecycle === "completed" ? "INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION_VERIFICATION_DUPLICATE_COMPLETED" : "INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION_VERIFICATION_DUPLICATE_IDENTITY", "Import contains duplicate execution verification identities."); importedVerificationIdentities.add(verificationIdentity);
        let state = entry.state;
        if (collision) {
          const referenceMap = new Map([[sourceProjectId, projectId], ...calculationMap.entries(), ...progressMap.entries()]);
          for (let index = 0; index < progress.length; index += 1) { const beforeState = progress[index]?.state; const afterState = importedProgress[index]?.state; for (const field of ["id", "initiationId", "proposalId", "reviewId", "decisionId", "executionId", "verificationId"]) if (beforeState?.[field] && afterState?.[field]) referenceMap.set(beforeState[field], afterState[field]); }
          state = clone(verificationApi.remapPatternEvolutionExecutionVerification(state, referenceMap));
        }
        const linkedExecution = importedProgress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_EXECUTION" && candidate.state?.id === state.sourceExecutionId); if (!linkedExecution) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_EXECUTION_VERIFICATION_REFERENCE", "Verification references a missing pattern evolution execution.");
        if (!state.importedUnproven) state = clone(verificationApi.makeImportedPatternEvolutionExecutionVerificationUnproven(state, { now: state.updatedAt, collision, reason: "import_identity_unproven" })); entry.state = state; validateImportedPatternEvolutionExecutionVerification(state, projectId);
      });
      const importedAcceptanceIdentities = new Set();
      importedProgress.forEach((entry, entryIndex) => {
        if (entry.kind !== "PATTERN_EVOLUTION_ACCEPTANCE") return;
        const acceptanceApi = global.YarnAIPatternEvolutionAcceptance;
        if (!acceptanceApi?.remapPatternEvolutionAcceptance || !acceptanceApi?.makeImportedPatternEvolutionAcceptanceUnproven || !acceptanceApi?.calculateAcceptanceIdentity) throw new ProjectRepositoryError("PATTERN_EVOLUTION_ACCEPTANCE_API_MISSING", "Pattern evolution acceptance module is not loaded.");
        const originalState = progress[entryIndex]?.state;
        const sourceKinds = [["verification", "PATTERN_EVOLUTION_EXECUTION_VERIFICATION", "sourceVerificationId"], ["execution", "PATTERN_EVOLUTION_EXECUTION", "sourceExecutionId"], ["decision", "PATTERN_EVOLUTION_DECISION", "sourceDecisionId"], ["review", "PATTERN_EVOLUTION_PROPOSAL_REVIEW", "sourceReviewId"], ["proposal", "PATTERN_EVOLUTION_PROPOSAL", "sourceProposalId"], ["initiation", "PATTERN_EVOLUTION_INITIATION", "sourceInitiationId"]];
        for (const [name, kind, idField] of sourceKinds) { const linked = progress.find((candidate) => candidate.kind === kind && candidate.state?.id === originalState?.[idField]); if (!linked || canonicalize(originalState.sourceSnapshots?.[name]) !== canonicalize(linked.state)) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_ACCEPTANCE_SOURCE_SNAPSHOT", "Acceptance source snapshots do not match the imported immutable source chain."); }
        const acceptanceIdentity = acceptanceApi.calculateAcceptanceIdentity(originalState); if (importedAcceptanceIdentities.has(acceptanceIdentity)) throw new ProjectRepositoryError(acceptanceApi.TERMINAL_LIFECYCLES.includes(originalState.lifecycle) ? "INVALID_IMPORT_PATTERN_EVOLUTION_ACCEPTANCE_DUPLICATE_TERMINAL" : "INVALID_IMPORT_PATTERN_EVOLUTION_ACCEPTANCE_DUPLICATE_IDENTITY", "Import contains duplicate pattern evolution acceptance identities."); importedAcceptanceIdentities.add(acceptanceIdentity);
        let state = entry.state;
        if (collision) {
          const referenceMap = new Map([[sourceProjectId, projectId], ...calculationMap.entries(), ...progressMap.entries()]);
          for (let index = 0; index < progress.length; index += 1) { const beforeState = progress[index]?.state; const afterState = importedProgress[index]?.state; for (const field of ["id", "initiationId", "proposalId", "reviewId", "decisionId", "executionId", "verificationId", "acceptanceId"]) if (beforeState?.[field] && afterState?.[field]) referenceMap.set(beforeState[field], afterState[field]); for (const collection of ["acceptanceCriteria", "mandatoryConditions", "residualConditions", "postAcceptanceObligations"]) for (let itemIndex = 0; itemIndex < (beforeState?.acceptanceContract?.[collection] || []).length; itemIndex += 1) { const beforeItem = beforeState.acceptanceContract[collection][itemIndex]; const afterItem = afterState?.acceptanceContract?.[collection]?.[itemIndex]; if (beforeItem?.id && afterItem?.id) referenceMap.set(beforeItem.id, afterItem.id); } }
          state = clone(acceptanceApi.remapPatternEvolutionAcceptance(state, referenceMap));
        }
        const linkedVerification = importedProgress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_EXECUTION_VERIFICATION" && candidate.state?.id === state.sourceVerificationId); if (!linkedVerification) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_ACCEPTANCE_REFERENCE", "Acceptance references a missing execution verification.");
        if (!state.importedUnproven) state = clone(acceptanceApi.makeImportedPatternEvolutionAcceptanceUnproven(state, { now: state.updatedAt, collision, reason: "import_identity_unproven" })); entry.state = state; validateImportedPatternEvolutionAcceptance(state, projectId);
      });
      const importedClosureIdentities = new Set();
      importedProgress.forEach((entry, entryIndex) => {
        if (entry.kind !== "PATTERN_EVOLUTION_CLOSURE") return;
        const closureApi = global.YarnAIPatternEvolutionClosure;
        if (!closureApi?.remapPatternEvolutionClosure || !closureApi?.makeImportedPatternEvolutionClosureUnproven || !closureApi?.calculateClosureIdentity) throw new ProjectRepositoryError("PATTERN_EVOLUTION_CLOSURE_API_MISSING", "Pattern evolution closure module is not loaded.");
        const originalState = progress[entryIndex]?.state;
        const sourceKinds = [["acceptance", "PATTERN_EVOLUTION_ACCEPTANCE", "sourceAcceptanceId"], ["verification", "PATTERN_EVOLUTION_EXECUTION_VERIFICATION", "sourceVerificationId"], ["execution", "PATTERN_EVOLUTION_EXECUTION", "sourceExecutionId"], ["decision", "PATTERN_EVOLUTION_DECISION", "sourceDecisionId"], ["review", "PATTERN_EVOLUTION_PROPOSAL_REVIEW", "sourceReviewId"], ["proposal", "PATTERN_EVOLUTION_PROPOSAL", "sourceProposalId"], ["initiation", "PATTERN_EVOLUTION_INITIATION", "sourceInitiationId"]];
        for (const [name, kind, idField] of sourceKinds) { const linked = progress.find((candidate) => candidate.kind === kind && candidate.state?.id === originalState?.[idField]); if (!linked || canonicalize(originalState.sourceSnapshots?.[name]) !== canonicalize(linked.state)) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_CLOSURE_SOURCE_SNAPSHOT", "Closure source snapshots do not match the imported immutable live chain."); }
        const closureIdentity = closureApi.calculateClosureIdentity(originalState); if (importedClosureIdentities.has(closureIdentity)) throw new ProjectRepositoryError(closureApi.TERMINAL_LIFECYCLES.includes(originalState.lifecycle) ? "INVALID_IMPORT_PATTERN_EVOLUTION_CLOSURE_DUPLICATE_TERMINAL" : "INVALID_IMPORT_PATTERN_EVOLUTION_CLOSURE_DUPLICATE_IDENTITY", "Import contains duplicate pattern evolution closure identities."); importedClosureIdentities.add(closureIdentity);
        let state = entry.state;
        if (collision) {
          const referenceMap = new Map([[sourceProjectId, projectId], ...calculationMap.entries(), ...progressMap.entries()]);
          for (let index = 0; index < progress.length; index += 1) { const beforeState = progress[index]?.state; const afterState = importedProgress[index]?.state; for (const field of ["id", "initiationId", "proposalId", "reviewId", "decisionId", "executionId", "verificationId", "acceptanceId", "closureId"]) if (beforeState?.[field] && afterState?.[field]) referenceMap.set(beforeState[field], afterState[field]); }
          state = clone(closureApi.remapPatternEvolutionClosure(state, referenceMap));
        }
        const linkedAcceptance = importedProgress.find((candidate) => candidate.kind === "PATTERN_EVOLUTION_ACCEPTANCE" && candidate.state?.id === state.sourceAcceptanceId); if (!linkedAcceptance) throw new ProjectRepositoryError("INVALID_IMPORT_PATTERN_EVOLUTION_CLOSURE_REFERENCE", "Closure references a missing acceptance.");
        if (!state.importedUnproven) state = clone(closureApi.makeImportedPatternEvolutionClosureUnproven(state, { now: state.updatedAt, collision, reason: "import_identity_unproven" })); entry.state = state; validateImportedPatternEvolutionClosure(state, projectId);
      });
